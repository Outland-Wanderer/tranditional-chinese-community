translate tranditional_chinese Split_Trail_Loop_0b620d89:

    # "You come across a carrot plant underneath the grass."
    ""

translate tranditional_chinese Split_Trail_Loop_17da5b77:

    # "You picked up a carrot, the plant will probably replenish in a few hours."
    ""

translate tranditional_chinese Split_Trail_Loop_d90591c2:

    # "There doesn't seem to be any carrot in this plant, maybe come back later..."
    ""

translate tranditional_chinese Split_Trail_Loop_9b2f2b1b:

    # "You are not sure... if you should leave with the flower in your hand."
    ""

translate tranditional_chinese Split_Trail_Loop_2dd49d8c:

    # msg "Do you want to leave the area, states of the dungeon will not be saved." nointeract
    msg "" nointeract

translate tranditional_chinese Split_Trail_Loop_a878fa1e:

    # "Reading... the sign carefully, you notice there is a few lines hidden in the corner of the sign."
    ""

translate tranditional_chinese Split_Trail_Loop_1e81db67:

    # "It says..."
    ""

translate tranditional_chinese Split_Trail_Loop_057b2615:

    # "{i}To receive the bow of the hunter, one must place barrels around a true carrot in all sides and corners...{/i}"
    ""

translate tranditional_chinese Split_Trail_Loop_b5709d4d:

    # "The carrot plant near the sign suddenly convulses..."
    ""

translate tranditional_chinese Split_Trail_Loop_808f81da:

    # "A hunting bow drops from the carrot plant, you thank whoever is granting you the gift, and pick it up."
    ""

translate tranditional_chinese Split_Trail_Loop_3ec1d6c4:

    # "It seems you already own the bow, the bush slowly stops convulsing..."
    ""

translate tranditional_chinese Split_Trail_Loop_56bc5980:

    # "There seems to be something underneath the text, maybe you should come back later... after visiting the cavern."
    ""

translate tranditional_chinese Split_Trail_Loop_509dcf5e:

    # "The flower withers in your hand..."
    ""

translate tranditional_chinese Split_Trail_Loop_cc558204:

    # "There is a flower... on the ground."
    ""

translate tranditional_chinese Split_Trail_Loop_f093c0a7:

    # "You notice the bush is blocking the pathway in front of you..."
    ""

translate tranditional_chinese Split_Trail_Loop_08d1e649:

    # "There's a small drawing in the middle of the bush..."
    ""

translate tranditional_chinese Split_Trail_Loop_bb0ec76a:

    # "Something... flower. And something... in the pot?"
    ""

translate tranditional_chinese Split_Trail_Loop_e4bc7423:

    # "You quickly put the drawing back, perhaps there's something that can remove this bush."
    ""

translate tranditional_chinese Split_Trail_Loop_6fbeccd5:

    # "You enter the northeast of the split trail."
    ""

translate tranditional_chinese Split_Trail_Loop_9b2f2b1b_1:

    # "You are not sure... if you should leave with the flower in your hand."
    ""

translate tranditional_chinese Split_Trail_Loop_1635364a:

    # "You reach into the pathway in front of you, it seems to lead to the deeper part of the forest..."
    ""

translate tranditional_chinese Split_Trail_Loop_5a996af5:

    # "You follow the step... eventually, you notice an old well in front of you..."
    ""

translate tranditional_chinese Split_Trail_Loop_d89d2071:

    # "You walk into the slumbrous well."
    ""

translate tranditional_chinese Split_Trail_Loop_9b2f2b1b_2:

    # "You are not sure... if you should leave with the flower in your hand."
    ""

translate tranditional_chinese Split_Trail_Loop_1635364a_1:

    # "You reach into the pathway in front of you, it seems to lead to the deeper part of the forest..."
    ""

translate tranditional_chinese Split_Trail_Loop_dac3171f:

    # "You follow the step... eventually, you see a cave in front of you..."
    ""

translate tranditional_chinese Split_Trail_Loop_58eda9c8:

    # "You walk towards the cavern entrance..."
    ""

translate tranditional_chinese Split_Trail_Loop_52786787:

    # "There is a pot on the grass..."
    ""

translate tranditional_chinese Split_Trail_Loop_942390d6:

    # "It seems that a path has unlocked..."
    ""

translate tranditional_chinese Split_Trail_Werewolf_7b7a5e55:

    # "As you walk through the forest, you encountered a werewolf."
    ""

translate tranditional_chinese Split_Trail_Werewolf_aa740c79:

    # "Immediately, you begin to run towards the opposite direction, but soon a dark figure comes into your view."
    ""

translate tranditional_chinese Split_Trail_Werewolf_f7dd4053:

    # e "Fuck..."
    e ""

translate tranditional_chinese Split_Trail_Caproot_0b620d89:

    # "You come across a carrot plant underneath the grass."
    ""

translate tranditional_chinese Split_Trail_Caproot_4f2b7598:

    # "As you pick up the carrot, it appears that it takes a little more strength to pull out."
    ""

translate tranditional_chinese Split_Trail_Caproot_3daaa82a:

    # "You use the strength of your body to pull up the carrot, only to clumsily fall on the ground."
    ""

translate tranditional_chinese Split_Trail_Caproot_4aef5d77:

    # "As you look up, you only see the carrot begins to strech itself... it is definitely not a carrot."
    ""

translate tranditional_chinese Split_Trail_Caproot_827cb48b:

    # "The Caproot monster is now almost bigger than twice your size... you need to defend yourself."
    ""

translate tranditional_chinese Chelforte_Cavern_Enter_ebd85903:

    # "The... entrance to the cave seems to be blocked for now..."
    ""

translate tranditional_chinese Chelforte_Cavern_Loop_88c40028:

    # "You jump into the water and quickly leave the cave..."
    ""

translate tranditional_chinese Chelforte_Cavern_Loop_9cdb018f:

    # "You look back, Wuldon and Vurro are still working, you have to be quick before returning to them again."
    ""

translate tranditional_chinese Chelforte_Cavern_Loop_c4ff8585:

    # "You find a small hole partway up the wall at the end of the cavern. Judging by the small pile of rocks at its foot, it was revealed after the cavern collapsed."
    ""

translate tranditional_chinese Chelforte_Cavern_Loop_e6d22e0b:

    # msg "Work in Progress!"
    msg ""

translate tranditional_chinese Chelforte_Cavern_Loop_499a581c:

    # "There's still monsters nearby, you decide it's better not to get yourself stuck while the nosferats... swing their meat around the cave."
    ""

translate tranditional_chinese Chelforte_Cavern_Loop_717f859c:

    # "Deciding you shouldn't go in for now, you turn around to keep exploring for a while."
    ""

translate tranditional_chinese Chelforte_Cavern_Loop_3dc67d59:

    # "You look at the rubble in front of you..."
    ""

translate tranditional_chinese Chelforte_Cavern_Loop_a28bc05d:

    # "It doesn't seem like you can fit through it at all."
    ""

translate tranditional_chinese Chelforte_Cavern_Loop_4002b44e:

    # "You try to look at the Feral Werewolf... It is much different than the normal werewolves you're encountered..."
    ""

translate tranditional_chinese Chelforte_Cavern_Loop_9de04ace:

    # "The monster in front of you, there's a lack of compassion in his face... only beastly instinct."
    ""

translate tranditional_chinese Chelforte_Cavern_Loop_cf0329a9:

    # "You remember Wuldon telling you about his past... and seeing the lifeless husk of a man like him now, only brings you grief and remorse..."
    ""

translate tranditional_chinese Chelforte_Cavern_Loop_77d72b5d:

    # "Whoever he was, it doesn't matter now."
    ""

translate tranditional_chinese Chelforte_Cavern_Loop_bccb361f:

    # "The beast howls."
    ""

translate tranditional_chinese Chelforte_Cavern_Loop_c3797b68:

    # "It sprints towards you with full force, there's no time to think now..."
    ""

translate tranditional_chinese Chelforte_Cavern_Loop_32931211:

    # "The entrance has been sealed tight since the cave-in from your last visit..."
    ""

translate tranditional_chinese Chelforte_Cavern_Loop_7c732532:

    # "You look at the tight hole in front of you, you notice there's a moving figure on the other side of the rubble..."
    ""

translate tranditional_chinese Chelforte_Cavern_Loop_721bf256:

    # "The hole in front of you is too small to fit through."
    ""

translate tranditional_chinese Chelforte_Cavern_Loop_13094814:

    # "Perhaps... if you have enough Agility, you can maybe fit through the hole..."
    ""

translate tranditional_chinese Chelforte_Cavern_Loop_cd2e94dc:

    # "But it's very possible you might get stuck in the hole while the figure is approaching you."
    ""

translate tranditional_chinese Chelforte_Cavern_Loop_63ec601c:

    # "You crawl into the tight hole, exhaling all air inside of you..."
    ""

translate tranditional_chinese Chelforte_Cavern_Loop_8a600863:

    # "Regardless of how much you breath, you soon realises you cannot make it."
    ""

translate tranditional_chinese Chelforte_Cavern_Loop_8a074efa:

    # "In the attempt to escape, you lost 30 health."
    ""

translate tranditional_chinese Chelforte_Cavern_Loop_001621ee:

    # "Fortunately, you escaped unscathed, but you believe that you can make it in...if you try again maybe."
    ""

translate tranditional_chinese Chelforte_Cavern_Loop_e3c61ec7:

    # "You make it through, rather easily."
    ""

translate tranditional_chinese Chelforte_Cavern_Loop_dbd7a24f:

    # "Soon, you are at the other side of the hole, without the figure noticing."
    ""

translate tranditional_chinese Chelforte_Cavern_Loop_c55d1030:

    # "The Entrance to the cave seems to be blocked..."
    ""

translate tranditional_chinese Chelforte_Cavern_Loop_2dd49d8c:

    # msg "Do you want to leave the area, states of the dungeon will not be saved." nointeract
    msg "" nointeract

translate tranditional_chinese Chelforte_Discovery_6cc9490e:

    # "The hole is a bit too high up for you to reach easily."
    ""

translate tranditional_chinese Chelforte_Discovery_b74e38d1:

    # "Despite this, you muster up your courage, and jump up for the entrance."
    ""

translate tranditional_chinese Chelforte_Discovery_c6111082:

    # "As your hands grab the tunnel's mouth, you feel it begin to give under your weight."
    ""

translate tranditional_chinese Chelforte_Discovery_62ae172c:

    # "Panicked, you begin scrabbling your way up, half pulling-half launching yourself up into the tunnel."
    ""

translate tranditional_chinese Chelforte_Discovery_95f5d601:

    # "The tunnel's mouth is a good bit wider after all is said and done, but you've made it in."
    ""

translate tranditional_chinese Chelforte_Discovery_a1f4d607:

    # "The cave-in seems to have loosened up some of the rock around here. Looking closely, you can see hairline cracks in the stone beneath you, fractures caused by the vibrations left by the cave-in."
    ""

translate tranditional_chinese Chelforte_Discovery_6cf30ec1:

    # "Luckily for you, the rock above you seems perfectly stable, clearly unaffected by whatever made the rocks below susceptible to the crash."
    ""

translate tranditional_chinese Chelforte_Discovery_5d429dd0:

    # "The tunnel goes diagonally down, a brief little thing. As you make your way down the increasingly claustrophobic tunnel, you begin to notice a gentle blue glow coming from in front of you."
    ""

translate tranditional_chinese Chelforte_Discovery_2bc3aa10:

    # "When you finally pop out on the other side of the hole, you find yourself in a large cavern, a ceiling perfectly black with moss but for crystals on the roof, gently reflecting blue light from the pond on the other end of the room."
    ""

translate tranditional_chinese Chelforte_Discovery_92d2131d:

    # "Despite being deep underground, you feel like you are looking at the night sky. It is a mesmerizing sight, lights dancing in the crystals unceasingly."
    ""

translate tranditional_chinese Chelforte_Discovery_ac2d3b06:

    # "Snapping yourself out of your daze, you move forward to the source of light."
    ""

translate tranditional_chinese Chelforte_Discovery_60c676f7:

    # "As you thought, it is the water at the end of the cave that emits light. Whether it is the water itself, or some tiny creature within, you cannot tell."
    ""

translate tranditional_chinese Chelforte_Discovery_54adc15a:

    # "You should go report this to Vurro and Wuldon."
    ""

translate tranditional_chinese Chelforte_Discovery_854a09b9:

    # "Moving towards the hole in the wall, you realize a small mistake you've made. It's fairly easy to go down a tunnel. The inverse cannot be said."
    ""

translate tranditional_chinese Chelforte_Discovery_3959010e:

    # "As you try to move back into the hole despite that, you soon find yourself stuck."
    ""

translate tranditional_chinese Chelforte_Discovery_c59ca116:

    # "You try to move backwards, backing out of the tunnel, but find you cannot, as the rock presses down against your sides such that your arms find little purchase to push yourself backwards with."
    ""

translate tranditional_chinese Chelforte_Discovery_d930a4d2:

    # "At that moment, you realize where you are."
    ""

translate tranditional_chinese Chelforte_Discovery_62cec831:

    # "Trapped in a tiny tunnel inside of a cave prone to cave-ins."
    ""

translate tranditional_chinese Chelforte_Discovery_86b23af6:

    # "Suddenly the minor pricks of stones poking into your skin stop being as unimportant."
    ""

translate tranditional_chinese Chelforte_Discovery_2ada0d44:

    # "The dearth of space, to the point of trapping your arms of head, makes your breath pick up as you fight against a rising feeling of panic."
    ""

translate tranditional_chinese Chelforte_Discovery_b2bcf265:

    # "Stuck as you are, and having no other recourse, you begin to yell for Vurro and Wuldon."
    ""

translate tranditional_chinese Chelforte_Discovery_a8e781cf:

    # "Soon, you hear the echoes of distant footsteps growing near."
    ""

translate tranditional_chinese Chelforte_Discovery_592c6690:

    # w "-ne?!"
    w ""

translate tranditional_chinese Chelforte_Discovery_1d8aed5b:

    # e "I'm here! In the hole in the wall!"
    e ""

translate tranditional_chinese Chelforte_Discovery_7a79c259:

    # "Your voice is deafening in your ears, echoing off of the tunnel walls."
    ""

translate tranditional_chinese Chelforte_Discovery_5831190e:

    # "Upon hearing your voice, the boots approach even faster, until they stop only a few feet above your head."
    ""

translate tranditional_chinese Chelforte_Discovery_7be847b6:

    # w "Are you safe, little one?!"
    w ""

translate tranditional_chinese Chelforte_Discovery_f1ed9578:

    # "You've never heard the werewolf get this scared, he sounds nearly panicked with worry."
    ""

translate tranditional_chinese Chelforte_Discovery_d6a70a0b:

    # e "Mmph. Mouth is full of dirt, and I'm stuck in a hole. Otherwise, I'm fine."
    e ""

translate tranditional_chinese Chelforte_Discovery_de3eff2f:

    # w "No imminent danger?"
    w ""

translate tranditional_chinese Chelforte_Discovery_879634cc:

    # e "No."
    e ""

translate tranditional_chinese Chelforte_Discovery_68802571:

    # "You hear an exasperated sigh from above you."
    ""

translate tranditional_chinese Chelforte_Discovery_2a874f44:

    # w "Please don't scare me like that again."
    w ""

translate tranditional_chinese Chelforte_Discovery_deb1149f:

    # "There is nothing you can say to that."
    ""

translate tranditional_chinese Chelforte_Discovery_0842ff34:

    # w "Are you in danger?"
    w ""

translate tranditional_chinese Chelforte_Discovery_e83f6bf0:

    # e "Not any immediate danger, no."
    e ""

translate tranditional_chinese Chelforte_Discovery_b92dc53d:

    # w "That's good."
    w ""

translate tranditional_chinese Chelforte_Discovery_9e33da31:

    # w "I assume you're just stuck?"
    w ""

translate tranditional_chinese Chelforte_Discovery_09d52514:

    # "You try to wiggle your arms, and find once again your space is too small for you to even readjust slightly."
    ""

translate tranditional_chinese Chelforte_Discovery_f6e7974a:

    # e "Yes, but it's really quite uncomfortable. Is there some way you could get me out?"
    e ""

translate tranditional_chinese Chelforte_Discovery_1978becf:

    # w "It's possible to dig you out, but it would take a while."
    w ""

translate tranditional_chinese Chelforte_Discovery_c0e17316:

    # v "We could just leave him down there for a while to see if he learns."
    v ""

translate tranditional_chinese Chelforte_Discovery_4b91c5d5:

    # "It seems Vurro is here too."
    ""

translate tranditional_chinese Chelforte_Discovery_d15f6b5e:

    # "Judging from the dull thwack above you, it also seems that Wuldon has made very clear to Vurro what he thinks of the idea."
    ""

translate tranditional_chinese Chelforte_Discovery_d978df91:

    # v "I was kidding, of course I wouldn't leave him down here."
    v ""

translate tranditional_chinese Chelforte_Discovery_0a506840:

    # w "That's why I hit you softly."
    w ""

translate tranditional_chinese Chelforte_Discovery_f605e5d0:

    # e "Hey, can you two bicker later please? I'd really like to get out now!"
    e ""

translate tranditional_chinese Chelforte_Discovery_731cb499:

    # "There's a brief pause. You can almost feel the two of them looking down at you from above."
    ""

translate tranditional_chinese Chelforte_Discovery_e22ba4a8:

    # w "Maybe we should just leave him here."
    w ""

translate tranditional_chinese Chelforte_Discovery_ff8c49a6:

    # e "Please don't."
    e ""

translate tranditional_chinese Chelforte_Discovery_12bab30b:

    # "You hear a soft snort from outside the tunnel."
    ""

translate tranditional_chinese Chelforte_Discovery_785935a7:

    # w "Fine, fine. We'll get you out."
    w ""

translate tranditional_chinese Chelforte_Discovery_716cc667:

    # w "Give us a second to grab our shovels – the ground here seems loose enough for us to dig you out easily."
    w ""

translate tranditional_chinese Chelforte_Discovery_9f09bd66:

    # e "Can you stay, please? I don't want to be left alone down here."
    e ""

translate tranditional_chinese Chelforte_Discovery_c83a13e1:

    # w "... Okay. I'll sit down next to the tunnel for you."
    w ""

translate tranditional_chinese Chelforte_Discovery_0a4b1a09:

    # "You hear a quiet shifting sound, followed by a soft whumph as the werewolf sits down."
    ""

translate tranditional_chinese Chelforte_Discovery_70496631:

    # w "Sorry to trouble you, Vurro, but could you-"
    w ""

translate tranditional_chinese Chelforte_Discovery_ac37fc8b:

    # v "Don't worry about it, I'll go get the shovels."
    v ""

translate tranditional_chinese Chelforte_Discovery_333d60a7:

    # "You hear the other werewolf's footsteps grow distant. Soon, It's just you and Wuldon here."
    ""

translate tranditional_chinese Chelforte_Discovery_580403c9:

    # "Neither of you say anything. Despite that, you feel much safer. You can hear Wuldon's breathing, the small rustles of his movement."
    ""

translate tranditional_chinese Chelforte_Discovery_0738f19a:

    # "You know he's here, and that he's staying vigilant for you."
    ""

translate tranditional_chinese Chelforte_Discovery_0fe4f290:

    # "Before you realize it, Vurro is already back with the shovels."
    ""

translate tranditional_chinese Chelforte_Discovery_30f9e4e5:

    # "You hear the footsteps of somebody going to find the shovels."
    ""

translate tranditional_chinese Chelforte_Discovery_5cbb563f:

    # v "Hey. I decided to stay behind and make sure you got out okay."
    v ""

translate tranditional_chinese Chelforte_Discovery_b1115493:

    # e "Shouldn't you be with Wuldon right now?"
    e ""

translate tranditional_chinese Chelforte_Discovery_6a3c6911:

    # "You get the distinct feeling that Vurro is waving his hand at that, dismissing the thought outright."
    ""

translate tranditional_chinese Chelforte_Discovery_bceb1302:

    # v "He'll get the shovels for me."
    v ""

translate tranditional_chinese Chelforte_Discovery_89b5ed4c:

    # "The two of you wait in awkward silence. You feel a rock digging into your stomach."
    ""

translate tranditional_chinese Chelforte_Discovery_5589205c:

    # v "You comfortable down there?"
    v ""

translate tranditional_chinese Chelforte_Discovery_9e175e04:

    # e "No. I'd really prefer to be out of here, if I'm being honest."
    e ""

translate tranditional_chinese Chelforte_Discovery_9adc788b:

    # v "Well, you're gonna be down there for a bit. Do you mind if I ask you a serious question?"
    v ""

translate tranditional_chinese Chelforte_Discovery_09d94b0f:

    # v "I'd like to learn a bit more about the person who saved me."
    v ""

translate tranditional_chinese Chelforte_Discovery_45808c0c:

    # e "Sure, I don't see why not."
    e ""

translate tranditional_chinese Chelforte_Discovery_616a093e:

    # "If you could hear a smile, you're pretty sure you did just now."
    ""

translate tranditional_chinese Chelforte_Discovery_5506c81a:

    # v "Great!"
    v ""

translate tranditional_chinese Chelforte_Discovery_854aafac:

    # v "So, do you have any hopes for the future?"
    v ""

translate tranditional_chinese Chelforte_Discovery_5fc16bf2:

    # "Starting out swinging, huh. Makes sense for a dead man, all things considered."
    ""

translate tranditional_chinese Chelforte_Discovery_47a9e1cb:

    # e "What do you mean exactly?"
    e ""

translate tranditional_chinese Chelforte_Discovery_443b51d3:

    # v "Whatever you want it to mean, really. For example, is there something you want to have happen between the werewolves and Lusterfield?"
    v ""

translate tranditional_chinese Chelforte_Discovery_371ac666:

    # "Yet again, Vurro is asking questions that you haven't really given much thought."
    ""

translate tranditional_chinese Chelforte_Discovery_d46f5f38:

    # e "I... don't know."
    e ""

translate tranditional_chinese Chelforte_Discovery_afc2fa28:

    # v "Come on, there has to be {i}something{/i}."
    v ""

translate tranditional_chinese Chelforte_Discovery_72a33094:

    # e "..."
    e ""

translate tranditional_chinese Chelforte_Discovery_de87e21d:

    # e "I want to know what caused the war with the goats. Why the magic is disappearing."
    e ""

translate tranditional_chinese Chelforte_Discovery_58649503:

    # e "Why I'm here."
    e ""

translate tranditional_chinese Chelforte_Discovery_51ea47d8:

    # "You hear a chuckle from above you."
    ""

translate tranditional_chinese Chelforte_Discovery_4ce6e4fc:

    # v "I think Wuldon's coming back, so I'll keep it short, but..."
    v ""

translate tranditional_chinese Chelforte_Discovery_25ed9f30:

    # v "I hope you figure it out."
    v ""

translate tranditional_chinese Chelforte_Discovery_a387861f:

    # "Vurro straightens up, and leans against the wall. At least you think he does."
    ""

translate tranditional_chinese Chelforte_Discovery_3711272b:

    # v "Took you long enough."
    v ""

translate tranditional_chinese Chelforte_Discovery_113af265:

    # w "I'm going to hit you over the head with this shovel."
    w ""

translate tranditional_chinese Chelforte_Discovery_3a587ff2:

    # "Both voices are extremely close above you now. It seems Wuldon walked over without you hearing his footsteps."
    ""

translate tranditional_chinese Chelforte_Discovery_d49b2f1f:

    # w "I'd recommend you cover your ears if you can. If not... my condolences."
    w ""

translate tranditional_chinese Chelforte_Discovery_44db0946:

    # "Having said that Wuldon grips his shovel and slams it into the ground at full force. At least, you hope it's full force, as the metallic clang that fills your ears makes it feel like it was your skull that was split in two."
    ""

translate tranditional_chinese Chelforte_Discovery_3e519f96:

    # "Immediately following is another earsplitting peal, as Vurro's shovel hits the ground."
    ""

translate tranditional_chinese Chelforte_Discovery_3b68b900:

    # "For hours, all you can hear is the banging of metal on stone, floor steadily making way for the werewolves."
    ""

translate tranditional_chinese Chelforte_Discovery_eb74b22a:

    # "As is smart of them, they dig diagonally down, using the cave's natural structure to reduce the work required."
    ""

translate tranditional_chinese Chelforte_Discovery_12959514:

    # "Eventually, the tip of Wuldon's boot enters your view."
    ""

translate tranditional_chinese Chelforte_Discovery_4abb79e4:

    # e "Be careful for this part! You're getting close to me!"
    e ""

translate tranditional_chinese Chelforte_Discovery_36202dbc:

    # "All you get are tired grunts of assent."
    ""

translate tranditional_chinese Chelforte_Discovery_17e78cd2:

    # "Even if the floor is weaker after the cave-in, it's still a fast pace, and enormous amount of work the two are undertaking for you."
    ""

translate tranditional_chinese Chelforte_Discovery_10bf244a:

    # "Luckily, they're also nearing the cave you found."
    ""

translate tranditional_chinese Chelforte_Discovery_9f05b6d0:

    # "The sound of metal on rock is far gentler now. Their shovels still pierce the earth, but with a delicacy previously absent."
    ""

translate tranditional_chinese Chelforte_Discovery_3a5a6589:

    # "Your ears still ache, but there's room for thought in your brain now, your voice no longer drowned out by the scream of metal."
    ""

translate tranditional_chinese Chelforte_Discovery_a9042998:

    # "The hole is still eminently uncomfortable, however. The closer the shovels get, the more dirt and stone falls from the ceiling onto your head."
    ""

translate tranditional_chinese Chelforte_Discovery_97bf0d94:

    # "Within the first 30 minutes, the first shovel breaks ground above you."
    ""

translate tranditional_chinese Chelforte_Discovery_62e60f3f:

    # "Already you feel relief as a part of your prison breaks. You rotate your wrists gently, stretching your body where you couldn't before."
    ""

translate tranditional_chinese Chelforte_Discovery_69b7db16:

    # "Bit by bit the shovels uncover and more of you. First your hands, then your head, and finally, your shoulders."
    ""

translate tranditional_chinese Chelforte_Discovery_1e1e11b5:

    # "As soon as they reach a part of your body they can reach, the shovelling stops."
    ""

translate tranditional_chinese Chelforte_Discovery_e983b468:

    # "You feel a strong pair of hands reach down and tug you gently. It is difficult not to cry out in pain as your belly scrapes the ground, jagged pieces of rock dragging through your skin."
    ""

translate tranditional_chinese Chelforte_Discovery_5b0a2e91:

    # "Half of your body is out now. The hands continue pulling you, but another pair begins patting down your back."
    ""

translate tranditional_chinese Chelforte_Discovery_3785103d:

    # v "You really need to be cleaned, you look like a statue."
    v ""

translate tranditional_chinese Chelforte_Discovery_379c25e7:

    # e "Having rocks and dust rain on you for hours on end will do that to you."
    e ""

translate tranditional_chinese Chelforte_Discovery_8eb229a5:

    # "As your legs finally leave the tunnel, your body completely free of the tunnel, you find yourself thrown onto someone's - Wuldon - back like a sack of potatoes."
    ""

translate tranditional_chinese Chelforte_Discovery_e651ec1a:

    # e "It's alright! I'm out! I'm out! My legs still work, Wuldon!"
    e ""

translate tranditional_chinese Chelforte_Discovery_604bd8f9:

    # "Wuldon begins running to the mouth of the tunnel, as if you had said nothing."
    ""

translate tranditional_chinese Chelforte_Discovery_4f6196fd:

    # w "I know, but you're filthy, and now we are too."
    w ""

translate tranditional_chinese Chelforte_Discovery_dcfdd6b9:

    # "A confusing statement, but one you're too tired to argue with right now."
    ""

translate tranditional_chinese Chelforte_Discovery_4217a17d:

    # "The only sounds in the tunnel are the echoes of Wuldon's feet on the ground, a rythmic beat steady as the tick of a clock."
    ""

translate tranditional_chinese Chelforte_Discovery_3854316e:

    # "It isn't long before you find yourself staring at the pool of water you entered from."
    ""

translate tranditional_chinese Chelforte_Discovery_1a7c45b3:

    # w "You good to go in?"
    w ""

translate tranditional_chinese Chelforte_Discovery_abc4599f:

    # e "You're not going to throw me or something?"
    e ""

translate tranditional_chinese Chelforte_Discovery_0090c603:

    # "Wuldon shakes his head, his fur brushing against your stomach."
    ""

translate tranditional_chinese Chelforte_Discovery_b6536947:

    # w "I think you deserve a break. Being stuck in a hole isn't fun."
    w ""

translate tranditional_chinese Chelforte_Discovery_5b5d3a9f:

    # "A sigh escapes your lips as Wuldon puts you down."
    ""

translate tranditional_chinese Chelforte_Discovery_d53a6b79:

    # e "It isn't."
    e ""

translate tranditional_chinese Chelforte_Discovery_9091bd0b:

    # e "You werewolves seem to like the experience though."
    e ""

translate tranditional_chinese Chelforte_Discovery_7a074a4b:

    # "It takes Wuldon a second to get it, at which point he laughs uproariously."
    ""

translate tranditional_chinese Chelforte_Discovery_cb2364c7:

    # w "Some don't - I'm glad I've never had to deal with having a knot, that's for sure."
    w ""

translate tranditional_chinese Chelforte_Discovery_d80387a3:

    # "You raise an eyebrow at that as Wuldon swings his arm around your side, pulling you into a semi-casual half hug."
    ""

translate tranditional_chinese Chelforte_Discovery_d34f1092:

    # w "God knows it would make going multiple rounds or cuddling harder."
    w ""

translate tranditional_chinese Chelforte_Discovery_22984b3a:

    # "As you struggle to find a proper answer to that, Wuldon lets go off you."
    ""

translate tranditional_chinese Chelforte_Discovery_cd5b0928:

    # w "Well. Let's get clean, shall we?"
    w ""

translate tranditional_chinese Chelforte_Discovery_d402cd4f:

    # "The blue werewolf immediately cannonballs into the water, covering your entire lower half with water."
    ""

translate tranditional_chinese Chelforte_Discovery_209f2355:

    # "Inspired by this, you take a running start and do the same."
    ""

translate tranditional_chinese Chelforte_Discovery_62952ca2:

    # "You can feel the rocks leave your fur as you meet the water. The initial dive was enough to return your skin to its brown color, rather than the dull gray of before."
    ""

translate tranditional_chinese Chelforte_Discovery_9cf795fd:

    # "Sinking down into the dark depths, you take a moment to rub your body, pushing loose more and more rocks until you feel acceptably rock free."
    ""

translate tranditional_chinese Chelforte_Discovery_1f21cbdc:

    # "Once done, you reach for the surface, swimming towards the torchlight."
    ""

translate tranditional_chinese Chelforte_Discovery_b5491e4e:

    # "Your head surges free of the water as you take a breath."
    ""

translate tranditional_chinese Chelforte_Discovery_ee558b92:

    # e "Won't we not be able to breath if you put up a torch?"
    e ""

translate tranditional_chinese Chelforte_Discovery_2ef5ea12:

    # "The blue werewolf gives you a small shrug."
    ""

translate tranditional_chinese Chelforte_Discovery_840880d4:

    # w "I made a hole in the ceiling, remember?"
    w ""

translate tranditional_chinese Chelforte_Discovery_7964bb37:

    # w "Not sure if it's enough to supply three people and a torch, but only one way to find out, right?"
    w ""

translate tranditional_chinese Chelforte_Discovery_f90b4630:

    # "Unconcerned by the look of horror on your face, Wuldon turns back to the tunnel they got you out of. After a few moments of shock, you give choice."
    ""

translate tranditional_chinese Chelforte_Discovery_306508b6:

    # e "Hey, you're kidding, right?"
    e ""

translate tranditional_chinese Chelforte_Discovery_d9725aca:

    # "Wuldon gives you a lighthearted chuckle."
    ""

translate tranditional_chinese Chelforte_Discovery_e2f9da0f:

    # w "What do you think, little one?"
    w ""

translate tranditional_chinese Chelforte_Discovery_288ed531:

    # e "That you're messing with me."
    e ""

translate tranditional_chinese Chelforte_Discovery_2a660715:

    # w "Attaboy."
    w ""

translate tranditional_chinese Chelforte_Discovery_66a4b614:

    # e "Were you going to say that regardless of what I said?"
    e ""

translate tranditional_chinese Chelforte_Discovery_e593ffb8:

    # "Wuldon grins mischievously, eyes shining playfully."
    ""

translate tranditional_chinese Chelforte_Discovery_b56b3bd7:

    # w "Maybe."
    w ""

translate tranditional_chinese Chelforte_Discovery_bd557ac5:

    # "You get the feeling that's the most you're getting out of him, at least right now."
    ""

translate tranditional_chinese Chelforte_Discovery_c004858b:

    # w "So, anything of interest in that tunnel?"
    w ""

translate tranditional_chinese Chelforte_Discovery_41702f8a:

    # "You give your best smirk."
    ""

translate tranditional_chinese Chelforte_Discovery_f1642409:

    # e "Maybe."
    e ""

translate tranditional_chinese Chelforte_Discovery_2bf90d56:

    # w "Ah, that's a yes. Good."
    w ""

translate tranditional_chinese Chelforte_Discovery_d7d98e72:

    # "Not responding is the best you can do to maintain even a shred of ambiguity. Even if Wuldon guessed correctly, he might still doubt himself."
    ""

translate tranditional_chinese Chelforte_Discovery_5ef4a8a1:

    # "The two of you eventually reach the tunnel's entrance, where a sleepy Vurro sits with his back to the wall."
    ""

translate tranditional_chinese Chelforte_Discovery_bd087adc:

    # "Wuldon bends down and gently shakes his friend's drowsiness away."
    ""

translate tranditional_chinese Chelforte_Discovery_bf14a88d:

    # w "Come on, we have to finish excavating the tunnel."
    w ""

translate tranditional_chinese Chelforte_Discovery_bcbcf61a:

    # "A halfway alert Vurro complies with his friend's demands, getting up and grabbing a shovel."
    ""

translate tranditional_chinese Chelforte_Discovery_c249a61a:

    # v "You can rest for a bit, [e], the tunnel is almost done, and I want to be the one to finish it. Well, with Wuldon, I mean."
    v ""

translate tranditional_chinese Chelforte_Discovery_abe08ab0:

    # "The other werewolf rolls his eyes and strikes the ground without ceremony."
    ""

translate tranditional_chinese Chelforte_Discovery_10923144:

    # "Vurro rushes after Wuldon and begins to do his part as well."
    ""

translate tranditional_chinese Chelforte_Discovery_ecba45f5:

    # "You drift off to the screech of the shovels, and the grunts of both werewolves pushing themselves."
    ""

translate tranditional_chinese Chelforte_Discovery_47f3b820:

    # "It's pretty awful background noise, but you're too tired to care right now."
    ""

translate tranditional_chinese Chelforte_Discovery_789ae6f1:

    # "The ground is softer than I thought..."
    ""

translate tranditional_chinese Chelforte_Discovery_a2cba0a2:

    # "Not... Compla..."
    ""

translate tranditional_chinese Chelforte_Discovery_a20cefa7:

    # "..."
    ""

translate tranditional_chinese Chelforte_Discovery_cfed2e52:

    # "The ground is back to being hard rock again, much to your chagrin."
    ""

translate tranditional_chinese Chelforte_Discovery_b117c302:

    # "You can't quite bring yourself to care. You're tired, and they found the cave. That's all that matters."
    ""

translate tranditional_chinese Chelforte_Discovery_2f321537:

    # w "Finally waking up, little one?"
    w ""

translate tranditional_chinese Chelforte_Discovery_06beacd0:

    # "He says this gently, as if afraid to disrupt your slumber."
    ""

translate tranditional_chinese Chelforte_Discovery_23b271ca:

    # e "Mrrg."
    e ""

translate tranditional_chinese Chelforte_Discovery_46cc3b0c:

    # "Wuldon chuckles softly at your sleepy tone."
    ""

translate tranditional_chinese Chelforte_Discovery_a3310f81:

    # "Looking over, you see him by your side, looking up at the roof of the cavern."
    ""

translate tranditional_chinese Chelforte_Discovery_9e816633:

    # w "It's a beautiful place you found here, little one."
    w ""

translate tranditional_chinese Chelforte_Discovery_01aa1423:

    # w "This is what I meant when I called you a lucky charm."
    w ""

translate tranditional_chinese Chelforte_Discovery_f5309ad4:

    # "You shift a bit on the floor, trying to find a softer spot."
    ""

translate tranditional_chinese Chelforte_Discovery_1e2c4240:

    # e "mrrf. Thanks..."
    e ""

translate tranditional_chinese Chelforte_Discovery_f4cba0cd:

    # "You stretch a bit, trying to work out the tension in your back. A few pops later, and your body floods with relief."
    ""

translate tranditional_chinese Chelforte_Discovery_4fdda857:

    # w "Looks like you're having trouble sleeping on the ground."
    w ""

translate tranditional_chinese Chelforte_Discovery_e4b2888b:

    # "The werewolf shifts closer, so that your shoulder and his just barely don't touch."
    ""

translate tranditional_chinese Chelforte_Discovery_e8953bbc:

    # w "You mind if I help with that?"
    w ""

translate tranditional_chinese Chelforte_Discovery_2cedc68f:

    # "Rather than speak, your response is to shift closer to Wuldon."
    ""

translate tranditional_chinese Chelforte_Discovery_287cee54:

    # "Wuldon shifts his body to face you, a small smile telling you all you need to know."
    ""

translate tranditional_chinese Chelforte_Discovery_91ecc23b:

    # "The blue werewolf wraps his arm across your front, as his other arm moves lower and grabs you by the legs."
    ""

translate tranditional_chinese Chelforte_Discovery_82be1317:

    # w "Here."
    w ""

translate tranditional_chinese Chelforte_Discovery_a0bc3f2d:

    # "Wuldon picks you up gently, and slowly pulls you flush against his chest, ready to let go if you make even the slightest protest."
    ""

translate tranditional_chinese Chelforte_Discovery_9b93fcf1:

    # "Letting it happen, you find yourself tucked next to the werewolf, his chin resting on your head between your horns."
    ""

translate tranditional_chinese Chelforte_Discovery_93aaab83:

    # "He's warm. From his soft belly to the muscley arm you use as a pillow, his entire body radiates heat, a roaring hearth in the body of a man."
    ""

translate tranditional_chinese Chelforte_Discovery_79edddef:

    # "His other arm rests around your belly, hand idly scratching the fur within its reach."
    ""

translate tranditional_chinese Chelforte_Discovery_b89ec288:

    # w "Better?"
    w ""

translate tranditional_chinese Chelforte_Discovery_6628e018:

    # "He already knows, but he wants to hear you say it. For pride, or to tease you, you can't tell."
    ""

translate tranditional_chinese Chelforte_Discovery_a38c9e6e:

    # "Too tired for words, you just nuzzle closer to Wuldon."
    ""

translate tranditional_chinese Chelforte_Discovery_18075836:

    # "It seems this is answer enough for him, as he lets out a contented sigh above you."
    ""

translate tranditional_chinese Chelforte_Discovery_3ad2bfa1:

    # "You hate to break the happy silence, but you had a few questions to ask the big wolf."
    ""

translate tranditional_chinese Chelforte_Discovery_27cd17eb:

    # "At least he can still hold you as he answers."
    ""

translate tranditional_chinese Chelforte_Discovery_5d70a179:

    # e "Sorry, I think it'd be better to try and figure it out on my own."
    e ""

translate tranditional_chinese Chelforte_Discovery_a4011cb9:

    # "The werewolf's content expression looks strained as you say that."
    ""

translate tranditional_chinese Chelforte_Discovery_6f73479c:

    # w "Alright. Good luck with that."
    w ""

translate tranditional_chinese Chelforte_Discovery_eb175b65:

    # "He shifts back to where he was."
    ""

translate tranditional_chinese Chelforte_Discovery_7fc8eccc:

    # "Quietly, you speak back up, each word a challenge to form."
    ""

translate tranditional_chinese Chelforte_Discovery_fc21b246:

    # e "Wuldon... where's Vurro."
    e ""

translate tranditional_chinese Chelforte_Discovery_818653e1:

    # w "He's checking out the minerals in the area."
    w ""

translate tranditional_chinese Chelforte_Discovery_36a3fc58:

    # w "They're fairly promising, apparently."
    w ""

translate tranditional_chinese Chelforte_Discovery_a638a145:

    # "This is punctuated by a massive yawn."
    ""

translate tranditional_chinese Chelforte_Discovery_e039e74d:

    # e "We... oof. We sleeping here?"
    e ""

translate tranditional_chinese Chelforte_Discovery_82ff00ca:

    # w "We were planning to. It's alright if you have things to attend to."
    w ""

translate tranditional_chinese Chelforte_Discovery_8099357d:

    # "Even if you wanted, you're too tired to go do them. It's been a long day."
    ""

translate tranditional_chinese Chelforte_Discovery_a9a6a397:

    # e "mrrg."
    e ""

translate tranditional_chinese Chelforte_Discovery_6f9f1db8:

    # w "Agreed."
    w ""

translate tranditional_chinese Chelforte_Discovery_f1ede524:

    # "Despite saying that, the two of you keep your eyes open for a bit, drinking in the mood."
    ""

translate tranditional_chinese Chelforte_Discovery_c18f640d:

    # "It was hard not to, considering the warm, hard lump poking into your back."
    ""

translate tranditional_chinese Chelforte_Discovery_02a72d67:

    # "Neither of you outright acknowledged it or your own erection, not willing to do more than flirt right now, tired as you are."
    ""

translate tranditional_chinese Chelforte_Discovery_ff92cae2:

    # "Of course, Wuldon didn't quite let you go to sleep without telling you he knew, as his hand drifted further and further down your belly, until it occasionally brushed your tip when he scratched you."
    ""

translate tranditional_chinese Chelforte_Discovery_4b2c9838:

    # "The only revenge you could get on the werewolf was readjusting slightly every so often to tug on his shaft a bit."
    ""

translate tranditional_chinese Chelforte_Discovery_f7e4dd5e:

    # "Every time you did, Wuldon would let out a small growl and hump gently into your back."
    ""

translate tranditional_chinese Chelforte_Discovery_27992cd5:

    # "Eventually, the two of you quiet down and return to cuddling without teasing."
    ""

translate tranditional_chinese Chelforte_Discovery_a9a8f7de:

    # "You can tell Wuldon is holding himself back from going further right now - likely because of Vurro - but his body slowly relaxes as the gentle glow of the cave lulls him into drowsiness."
    ""

translate tranditional_chinese Chelforte_Discovery_ade813dd:

    # "It's like laying under the night sky, crystalline rocks sparkling with thousands of colors like the stars far above."
    ""

translate tranditional_chinese Chelforte_Discovery_5e9dfc25:

    # "There is no moon to provide light, but the slight blue glow of the pond fits the role well, drowning the cave in calm."
    ""

translate tranditional_chinese Chelforte_Discovery_ef129b57:

    # "You don't know when you fell asleep. One moment you were looking at the walls, and the next you were woken up by Wuldon affectionately nuzzling your neck."
    ""

translate tranditional_chinese Chelforte_Discovery_c93ba645:

    # w "As much as I hate to say it, we need to get going, little one."
    w ""

translate tranditional_chinese Chelforte_Discovery_ab28e3dd:

    # "So saying, he pulls away from you and gets on his feet."
    ""

translate tranditional_chinese Chelforte_Discovery_af71426f:

    # "You don't know when you fell asleep. One moment you were looking at the walls, and the next you were woken up by Wuldon pushing you gently."
    ""

translate tranditional_chinese Chelforte_Discovery_1e7cdb55:

    # w "Come on, we should get going."
    w ""

translate tranditional_chinese Chelforte_Discovery_8462893c:

    # "Still a bit groggy, you get up and stretch a bit to wake up."
    ""

translate tranditional_chinese Chelforte_Discovery_50ed8b3a:

    # w "Vurro is at the cave entrance, sleeping."
    w ""

translate tranditional_chinese Chelforte_Discovery_1a4c232b:

    # "You look up at him in the middle of a stretch, feeling a bit ridiculous."
    ""

translate tranditional_chinese Chelforte_Discovery_4ea1b3fd:

    # e "Any reason why?"
    e ""

translate tranditional_chinese Chelforte_Discovery_128811de:

    # w "He wanted to give us room or something, apparently."
    w ""

translate tranditional_chinese Chelforte_Discovery_9bcab37b:

    # w "The snoring was the biggest reason for him, I think."
    w ""

translate tranditional_chinese Chelforte_Discovery_c77cf443:

    # "Wuldon checks to see if you're ready to go, and turns towards the exit."
    ""

translate tranditional_chinese Chelforte_Discovery_52e24e7f:

    # w "We should get going before his snoring causes another cave-in."
    w ""

translate tranditional_chinese Chelforte_Discovery_73d7b3cf:

    # "Together, you jog over to the water hole."
    ""

translate tranditional_chinese Chelforte_Discovery_f05954e4:

    # "Vurro's unconscious form comes into view far after you hear the snoring."
    ""

translate tranditional_chinese Chelforte_Discovery_6c741e9e:

    # "He looks like passed out on his feet, considering the absurd heap he forms."
    ""

translate tranditional_chinese Chelforte_Discovery_efe8be86:

    # "Wuldon, apparently not caring for his condition, grabs Vurro and dunks him into the water headfirst."
    ""

translate tranditional_chinese Chelforte_Discovery_ebf38758:

    # "Vurro comes back out spluttering and indignant. He is put back down on the ground, where he spends a good minute sulking."
    ""

translate tranditional_chinese Chelforte_Discovery_5b887d4d:

    # v "I know it's because it's hard to wake me up, but please don't scare me like that."
    v ""

translate tranditional_chinese Chelforte_Discovery_3a7e4a9e:

    # "Wuldon shrugs."
    ""

translate tranditional_chinese Chelforte_Discovery_be988d7a:

    # w "No promises."
    w ""

translate tranditional_chinese Chelforte_Discovery_f2dfed0a:

    # "Vurro just keeps on looking at Wuldon."
    ""

translate tranditional_chinese Chelforte_Discovery_0991b613:

    # w "Okay, okay, I'll move it down the list of ways to wake you up."
    w ""

translate tranditional_chinese Chelforte_Discovery_80c6eacb:

    # "Vurro looks for a bit longer, until he gives up and accepts it for what it is."
    ""

translate tranditional_chinese Chelforte_Discovery_583cd602:

    # v "Alright. Well, now that I'm awake, I have a few things to say."
    v ""

translate tranditional_chinese Chelforte_Discovery_ce22499c:

    # v "First off, the cave is an incredible find. Not only can we invite other tribes to visit, some of the ore in that area is incredibly rare."
    v ""

translate tranditional_chinese Chelforte_Discovery_cc712d78:

    # v "Second, I'm hungry. We need to head back home and eat."
    v ""

translate tranditional_chinese Chelforte_Discovery_f40cb36a:

    # w "What happened to our supplies?"
    w ""

translate tranditional_chinese Chelforte_Discovery_7f811e1e:

    # "Vurro scratches the back of his head, embarrassed."
    ""

translate tranditional_chinese Chelforte_Discovery_3c8edb50:

    # v "I uhh... ate them."
    v ""

translate tranditional_chinese Chelforte_Discovery_8ce062c7:

    # v "I got sudden and intense hunger pangs earlier, and next thing I know, I've eaten all of the food."
    v ""

translate tranditional_chinese Chelforte_Discovery_91e4a95c:

    # "Incredibly concerning, but there's nothing you can really do."
    ""

translate tranditional_chinese Chelforte_Discovery_8ff58ff8:

    # e "So we're headed back home?"
    e ""

translate tranditional_chinese Chelforte_Discovery_cf359301:

    # v "Yes."
    v ""

translate tranditional_chinese Chelforte_Discovery_74401855:

    # v "Sorry that I couldn't talk to you about what we mentioned earlier, but I guess when we meet to hunt Uffe is as good a time as any."
    v ""

translate tranditional_chinese Chelforte_Discovery_22e179b0:

    # "Doubtful, but it'll have to do."
    ""

translate tranditional_chinese Chelforte_Discovery_cff0bbdc:

    # e "It's alright. I'm sure you'll tell me someday."
    e ""

translate tranditional_chinese Chelforte_Discovery_31f2b853:

    # e "We could alwa-"
    e ""

translate tranditional_chinese Chelforte_Discovery_1e6922af:

    # "A loud gurgle comes from Vurro's stomach, interrupting your thoughts."
    ""

translate tranditional_chinese Chelforte_Discovery_85eaf53b:

    # e "Nevermind. Let's get you some food."
    e ""

translate tranditional_chinese Chelforte_Discovery_7f02fe7e:

    # "The three of you look at the water with slight dismay. There's nothing for it to get in, but the three of you have had enough water for a long time."
    ""

translate tranditional_chinese Chelforte_Discovery_14afba7d:

    # "Moving past the hesitance, the three of you take the first step home together and dive in."
    ""

translate tranditional_chinese Chelforte_Discovery_9ff1c7d1:

    # "Conversation is sparse on your journey home, thoughts occupied by the events of last night, or plans to take down Vurro."
    ""

translate tranditional_chinese Chelforte_Discovery_1d31e0ee:

    # "Before you know it, you're back at Wuldon's house."
    ""

translate tranditional_chinese Chelforte_Discovery_26b34d99:

    # v "Well, I guess we part ways for now."
    v ""

translate tranditional_chinese Chelforte_Discovery_6a109a94:

    # v "We'll be grabbing supplies or otherwise preparing to take down Uffe for the next few days. I recommend you do the same."
    v ""

translate tranditional_chinese Chelforte_Discovery_86ff6451:

    # v "When we're both ready, come back here to go out and kill the bastard."
    v ""

translate tranditional_chinese Chelforte_Discovery_4dc52f08:

    # "Vurro walks up to you and throws his arms around you."
    ""

translate tranditional_chinese Chelforte_Discovery_61ba5ae3:

    # v "Wuldon and I have been friends since we were little. He was the brother I never had."
    v ""

translate tranditional_chinese Chelforte_Discovery_52dbd08d:

    # "You almost mention Uffe, but stop yourself. It's sad to see a brother disown another, but in this case, it's perfectly justified."
    ""

translate tranditional_chinese Chelforte_Discovery_4ae3a0b4:

    # v "Thank you for helping us, and for giving Wuldon company."
    v ""

translate tranditional_chinese Chelforte_Discovery_c1bc130b:

    # v "It means more than you know, and I wish I had some way to even start to pay you back."
    v ""

translate tranditional_chinese Chelforte_Discovery_88b608ff:

    # v "But I don't. So all I can say is thank you."
    v ""

translate tranditional_chinese Chelforte_Discovery_37f7d903:

    # "You hug him back without a word. There is nothing you can say to that."
    ""

translate tranditional_chinese Chelforte_Discovery_5ac7dcbe:

    # "The two of you stay embraced for a long time, Vurro trying to make up for time he'll never have."
    ""

translate tranditional_chinese Chelforte_Discovery_2dc15d54:

    # "When it finally ends, Vurro backs off, looking... empty."
    ""

translate tranditional_chinese Chelforte_Discovery_fa453b82:

    # v "I'll see you in a few days."
    v ""

translate tranditional_chinese Chelforte_Discovery_c19fea77:

    # "As he says that, he gives you a sad smile, and heads inside."
    ""

translate tranditional_chinese Chelforte_Discovery_375e9e50:

    # "Wuldon is standing by the door somewhat awkwardly, a bittersweet smile on his mouth."
    ""

translate tranditional_chinese Chelforte_Discovery_b3b0fe34:

    # "He walks forwards until he's less than a foot away from you."
    ""

translate tranditional_chinese Chelforte_Discovery_516f9609:

    # "You look up at his face as it looks down at you. Gentle, is how you'd describe him right now."
    ""

translate tranditional_chinese Chelforte_Discovery_c1c4cb6f:

    # w "There is a lot I want to tell you."
    w ""

translate tranditional_chinese Chelforte_Discovery_c7877a2f:

    # "His face scrunches up in pain."
    ""

translate tranditional_chinese Chelforte_Discovery_92debfdb:

    # w "But I want to wait until after we fight Uffe. Otherwise..."
    w ""

translate tranditional_chinese Chelforte_Discovery_4fe238fc:

    # "You nod to him carefully, and beckon him down so you can whisper in his ears."
    ""

translate tranditional_chinese Chelforte_Discovery_aa0c7cb0:

    # e "I understand."
    e ""

translate tranditional_chinese Chelforte_Discovery_c01e1536:

    # "So saying, you grab both sides of Wuldon's head and gently kiss him on the cheek."
    ""

translate tranditional_chinese Chelforte_Discovery_77aa761b:

    # "When you pull back, Wuldon stays there, looking at you sadly."
    ""

translate tranditional_chinese Chelforte_Discovery_b5c9b02b:

    # w "You do."
    w ""

translate tranditional_chinese Chelforte_Discovery_19f2655c:

    # "The blue werewolf sighs as he stands back up."
    ""

translate tranditional_chinese Chelforte_Discovery_c4990880:

    # w "Stay safe. For both of us."
    w ""

translate tranditional_chinese Chelforte_Discovery_35a572e0:

    # "You give him a sad smile."
    ""

translate tranditional_chinese Chelforte_Discovery_7cfd1808:

    # e "I thought you said you'd keep me safe?"
    e ""

translate tranditional_chinese Chelforte_Discovery_a603ee09:

    # w "I said I'd try."
    w ""

translate tranditional_chinese Chelforte_Discovery_7b5add91:

    # "He pauses momentarily, gathering his thoughts."
    ""

translate tranditional_chinese Chelforte_Discovery_ded60609:

    # w "I wish I could promise I will."
    w ""

translate tranditional_chinese Chelforte_Discovery_ad944cdb:

    # "With great difficulty, you give him a reassuring smile."
    ""

translate tranditional_chinese Chelforte_Discovery_3445e722:

    # e "You do your best. Vurro and I know that."
    e ""

translate tranditional_chinese Chelforte_Discovery_23be8d4c:

    # "With a heavy heart, Wuldon turns around."
    ""

translate tranditional_chinese Chelforte_Discovery_cfbcb80c:

    # w "I know. But my best doesn't do much if my best friend is slated to die, and all I can do is watch."
    w ""

translate tranditional_chinese Chelforte_Discovery_72a33094_1:

    # e "..."
    e ""

translate tranditional_chinese Chelforte_Discovery_de0d00e1:

    # e "We brought him back for a while, and we're going to get revenge."
    e ""

translate tranditional_chinese Chelforte_Discovery_f432ca3d:

    # "Wuldon pauses at the doorway. When he finally speaks, his voice is tired, but determined."
    ""

translate tranditional_chinese Chelforte_Discovery_f8d21319:

    # w "Yes. I'm infinitely grateful for what I have, and I'll do my best to treasure it while I can. You've helped me do that, and now you're also helping me finish this whole thing."
    w ""

translate tranditional_chinese Chelforte_Discovery_823e59d7:

    # w "We will get revenge, and I will hunt down whover did this and make sure he can't fuck with what's mine anymore."
    w ""

translate tranditional_chinese Chelforte_Discovery_97e88f0e:

    # w "For that to happen, we both have to live. Sharpen your weapons, stock up on potions... do whatever you can to get ready."
    w ""

translate tranditional_chinese Chelforte_Discovery_905f9c94:

    # "Wuldon briefly turns to look at you again."
    ""

translate tranditional_chinese Chelforte_Discovery_48380a13:

    # w "I look forward to seeing you again when you are."
    w ""

translate tranditional_chinese Chelforte_Discovery_f3008618:

    # "And so, with a true, genuine smile on his face, Wuldon walked into his home, leaving you alone to process everything from the last few minutes."
    ""

translate tranditional_chinese Chelforte_Discovery_3646a7e7:

    # "It's time to go home."
    ""

translate tranditional_chinese Chelforte_Discovery_f02c0096:

    # "Wuldon is standing by the door somewhat awkwardly."
    ""

translate tranditional_chinese Chelforte_Discovery_54867737:

    # w "You know how I feel, but... thank you. Stay safe, little one."
    w ""

translate tranditional_chinese Chelforte_Discovery_69a90bf7:

    # e "I will. Gotta stay strong to fight Uffe, right?"
    e ""

translate tranditional_chinese Chelforte_Discovery_d73e09c8:

    # w "Yeah. It's going to be an uphill battle."
    w ""

translate tranditional_chinese Chelforte_Discovery_cf788e3b:

    # "Waving goodbye, Wuldon turns around and walks into his home, leaving you alone."
    ""

translate tranditional_chinese Chelforte_Discovery_ca35251b:

    # "It's time to head home."
    ""

translate tranditional_chinese Chelforte_Discovery_7cc84a7b:

    # msg "You received a level up point and 300 gold."
    msg ""

translate tranditional_chinese Chelforte_Nosferat_80b59c53:

    # "You encounter a nosferat..."
    ""

translate tranditional_chinese Chelforte_Wuldon_a391ad25:

    # "Wuldon turns around from slamming his pickaxe into the wall."
    ""

translate tranditional_chinese Chelforte_Wuldon_af4a74f6:

    # "Looking at him like this, you think you can understand why the farmers were scared of him."
    ""

translate tranditional_chinese Chelforte_Wuldon_93ce62f4:

    # "His muscles bulge underneath his fur, which is matted with the dust and sweat of hard work in a mine. All of this contextualized by the sharp metal pickaxe in his hands, and the hole forming in the solid stone wall."
    ""

translate tranditional_chinese Chelforte_Wuldon_060f6cf3:

    # w "You got any questions for me?"
    w ""

translate tranditional_chinese Chelforte_Wuldon_5db474b4:

    # e "Where do you think I should explore first?"
    e ""

translate tranditional_chinese Chelforte_Wuldon_b91fd81b:

    # "Wuldon gives you a shrug, putting his pickaxe on the ground, and leaning on it."
    ""

translate tranditional_chinese Chelforte_Wuldon_d5ea2fc5:

    # w "I don't know. That's why you're exploring, to figure out if there's anything worthwhile in this part of the cave."
    w ""

translate tranditional_chinese Chelforte_Wuldon_ed28b10c:

    # e "Anything else to keep in mind?"
    e ""

translate tranditional_chinese Chelforte_Wuldon_34255009:

    # w "Try to take out any threats you see along the way. I don't think any will try to kill you, but they'll definitely inconvenience anyone trying to work here."
    w ""

translate tranditional_chinese Chelforte_Wuldon_4357bfb3:

    # e "Alright. I'll be on my way then."
    e ""

translate tranditional_chinese Chelforte_Wuldon_918d29ca:

    # "Wuldon gives you a nod, hefting his pickaxe on his shoulder."
    ""

translate tranditional_chinese Chelforte_Wuldon_5bf98721:

    # w "Good luck."
    w ""

translate tranditional_chinese Chelforte_Wuldon_d3e9d18d:

    # w "You're lucky I waived the viewing fees for you."
    w ""

translate tranditional_chinese Chelforte_Wuldon_72a33094:

    # e "..."
    e ""

translate tranditional_chinese Chelforte_Wuldon_65eb36c3:

    # "You hadn't really realized you'd been staring. Wuldon doesn't seem to mind though, as he simply reaches out to ruffle your hair with a smile."
    ""

translate tranditional_chinese Chelforte_Wuldon_774434f8:

    # e "Fair enough. I originally came here because I wanted to ask you something, though."
    e ""

translate tranditional_chinese Chelforte_Wuldon_f1d90117:

    # w "Hmm?"
    w ""

translate tranditional_chinese Chelforte_Wuldon_4ba05d5a:

    # "Wuldon puts down his pickaxe for now, giving you his undivided attention."
    ""

translate tranditional_chinese Chelforte_Wuldon_5db474b4_1:

    # e "Where do you think I should explore first?"
    e ""

translate tranditional_chinese Chelforte_Wuldon_bceabb6d:

    # "The werewolf fidgets slightly, giving the question genuine thought."
    ""

translate tranditional_chinese Chelforte_Wuldon_fcc012ea:

    # w "I don't know. That's a lot of why we want your help exploring. You always seem to find something interesting."
    w ""

translate tranditional_chinese Chelforte_Wuldon_80e9ca7e:

    # e "So, I'm basically your good luck charm."
    e ""

translate tranditional_chinese Chelforte_Wuldon_4b9d8911:

    # "Wuldon nods, slightly sheepishly."
    ""

translate tranditional_chinese Chelforte_Wuldon_0eacedc6:

    # w "That, and you can fit in the spaces we can't."
    w ""

translate tranditional_chinese Chelforte_Wuldon_c5a39378:

    # e "No other reason I'm here?"
    e ""

translate tranditional_chinese Chelforte_Wuldon_95934182:

    # w "...you're good at fighting, and clearing out the threats for us would be extremely helpful?"
    w ""

translate tranditional_chinese Chelforte_Wuldon_19303305:

    # "You shake your head, knowing he's dodging the question."
    ""

translate tranditional_chinese Chelforte_Wuldon_4eb0adf8:

    # e "I know you can do all of this yourself with enough time and energy."
    e ""

translate tranditional_chinese Chelforte_Wuldon_d44bee6a:

    # w "That's... true. It's also not like I lack either of those."
    w ""

translate tranditional_chinese Chelforte_Wuldon_09727994:

    # "He glances at Vurro."
    ""

translate tranditional_chinese Chelforte_Wuldon_b3ef7511:

    # w "Well, normally. Right now we're on a timer with Vurro."
    w ""

translate tranditional_chinese Chelforte_Wuldon_7e4918bf:

    # w "But yes, you're right."
    w ""

translate tranditional_chinese Chelforte_Wuldon_6c719bee:

    # e "Right about what."
    e ""

translate tranditional_chinese Chelforte_Wuldon_b83b8ab8:

    # "The two of you know exactly what you mean, but Wuldon seems embarrassed to say it with Vurro nearby."
    ""

translate tranditional_chinese Chelforte_Wuldon_690e28da:

    # w "That we want you here."
    w ""

translate tranditional_chinese Chelforte_Wuldon_815e4040:

    # "You narrow your eyes at Wuldon, who fidgets unhappily."
    ""

translate tranditional_chinese Chelforte_Wuldon_4e4f5c51:

    # w "Yes, he wants you here mostly because I want you here. Are you happy now?"
    w ""

translate tranditional_chinese Chelforte_Wuldon_0f55e9ff:

    # e "As a matter of fact, I am."
    e ""

translate tranditional_chinese Chelforte_Wuldon_fcce1475:

    # w "...you're lucky I like you. Anyone else would have been laid flat by now."
    w ""

translate tranditional_chinese Chelforte_Wuldon_e5aa2963:

    # "Satisfied with what you've gotten, you turn around with a smile."
    ""

translate tranditional_chinese Chelforte_Wuldon_0f3202da:

    # e "Good thing you like me then."
    e ""

translate tranditional_chinese Chelforte_Wuldon_025bb35c:

    # "If he can mess with you, you can mess with him."
    ""

translate tranditional_chinese Chelforte_Wuldon_5356a0a2:

    # "The last thing you hear out of Wuldon before you start exploring again is a vaguely annoyed grumble."
    ""

translate tranditional_chinese Chelforte_Wuldon_0014f80e:

    # w "Didn't you just come here a second ago?"
    w ""

translate tranditional_chinese Chelforte_Wuldon_9f82fb28:

    # e "...Yeah. But I sort of forgot what I was looking for."
    e ""

translate tranditional_chinese Chelforte_Wuldon_ff23bb65:

    # "Wuldon shakes his head in the bemused confusion of a man watching a dog chase its own tail."
    ""

translate tranditional_chinese Chelforte_Wuldon_ce987370:

    # w "I don't know if I'm hoping that this is caused by a monster hitting you in the head, or not."
    w ""

translate tranditional_chinese Chelforte_Wuldon_412d9d9d:

    # w "Both options have worrying implications."
    w ""

translate tranditional_chinese Chelforte_Wuldon_af2cdd74:

    # "He's being a bastard as always."
    ""

translate tranditional_chinese Chelforte_Wuldon_77568443:

    # e "If it was the monster hitting me, it must have taken my memories of that too."
    e ""

translate tranditional_chinese Chelforte_Wuldon_a2d61b4b:

    # e "Now, please. Remind me of my task?"
    e ""

translate tranditional_chinese Chelforte_Wuldon_c16bfb9d:

    # "The werewolf gives a helpless shrug."
    ""

translate tranditional_chinese Chelforte_Wuldon_0d0c42dc:

    # w "You're exploring the nearby area for anything interesting, and clearing out any monsters you see along the way."
    w ""

translate tranditional_chinese Chelforte_Wuldon_a144760a:

    # e "Got it."
    e ""

translate tranditional_chinese Chelforte_Wuldon_de397181:

    # e "I'll talk to you later."
    e ""

translate tranditional_chinese Chelforte_Wuldon_c2888060:

    # w "Mmh. Hopefully not about the same question."
    w ""

translate tranditional_chinese Chelforte_Vurro_f04e4f94:

    # v "Oh, hey there, [e]. Good to see you survived the few minutes alone with Wuldon."
    v ""

translate tranditional_chinese Chelforte_Vurro_33120eff:

    # "As you dry yourself off, you see Vurro looking around somewhat vacantly."
    ""

translate tranditional_chinese Chelforte_Vurro_9fce886a:

    # e "Did the scouting go alright?"
    e ""

translate tranditional_chinese Chelforte_Vurro_698f37ff:

    # v "Mm. Yeah."
    v ""

translate tranditional_chinese Chelforte_Vurro_480bdadd:

    # "Vurro feels slightly... off."
    ""

translate tranditional_chinese Chelforte_Vurro_82634624:

    # e "You doing alright there?"
    e ""

translate tranditional_chinese Chelforte_Vurro_ff742d09:

    # "You get a gentle shake of the head from the brown werewolf."
    ""

translate tranditional_chinese Chelforte_Vurro_cf846ccb:

    # v "I'll be fine. It's just weird to be in this cave after everything that's happened."
    v ""

translate tranditional_chinese Chelforte_Vurro_5d6c8add:

    # v "...I don't like thinking about how many of my people are buried down here."
    v ""

translate tranditional_chinese Chelforte_Vurro_2a42b266:

    # "You both stand there in silence for a moment."
    ""

translate tranditional_chinese Chelforte_Vurro_84c45187:

    # v "You know, even now, I'm scared that I'll turn down here."
    v ""

translate tranditional_chinese Chelforte_Vurro_b12985a0:

    # "It's hard to know what to say to that. Luckily, you don't have to. A nearly invisible blue arm camouflaged by the water reaches out, and grabs Vurro's leg."
    ""

translate tranditional_chinese Chelforte_Vurro_0674c708:

    # "With a tremendous heave, Wuldon bursts out of the water, using his weight to pull Vurro down into the tunnel."
    ""

translate tranditional_chinese Chelforte_Vurro_8492bd8d:

    # "Wuldon lands sopping wet, but triumphant. He turns and waits for Vurro to appear."
    ""

translate tranditional_chinese Chelforte_Vurro_51c6c7f7:

    # "After a few short moments, an annoyed brown werewolf resurfaces, dry fur now returned to its soggy state."
    ""

translate tranditional_chinese Chelforte_Vurro_7e0af7a0:

    # v "You already threw me in once. Why do it again."
    v ""

translate tranditional_chinese Chelforte_Vurro_a2d8e44d:

    # "Wuldon gives a shrug."
    ""

translate tranditional_chinese Chelforte_Vurro_5ef4c3ae:

    # w "I could see that you were getting depressed again, so I thought I'd help you feel better."
    w ""

translate tranditional_chinese Chelforte_Vurro_65cabcbf:

    # v "..."
    v ""

translate tranditional_chinese Chelforte_Vurro_dd6a31fe:

    # v "Fine. But you're going to explain what we're doing down here to [e] while I dry off."
    v ""

translate tranditional_chinese Chelforte_Vurro_78e712e4:

    # w "Fair's fair."
    w ""

translate tranditional_chinese Chelforte_Vurro_9c521750:

    # "Wuldon turns to you."
    ""

translate tranditional_chinese Chelforte_Vurro_63f65e63:

    # w "Alright, little one. Our goals down here are to make an air vent, clear out some of the rubble disconnecting us from other areas, and find any sites that may interest us."
    w ""

translate tranditional_chinese Chelforte_Vurro_c146cfb1:

    # w "Vurro and I will mostly handle the first two, though he won't say no to extra help, if you want to give any."
    w ""

translate tranditional_chinese Chelforte_Vurro_0e1e5d72:

    # w "Your job will mainly be exploration. Having seen your performance in slime country, I have no doubt you'll find something interesting."
    w ""

translate tranditional_chinese Chelforte_Vurro_48549906:

    # w "Here's a pickaxe, you'll need it to get anything important."
    w ""

translate tranditional_chinese Chelforte_Vurro_b6cddd25:

    # w "Are we clear?"
    w ""

translate tranditional_chinese Chelforte_Vurro_ed7274fe:

    # "You give Wuldon a nod."
    ""

translate tranditional_chinese Chelforte_Vurro_50f664fa:

    # w "Perfect. Please stand back."
    w ""

translate tranditional_chinese Chelforte_Vurro_2898d4e9:

    # "Cautiously, you move as far away from Wuldon as you can. As soon as you cross some sort of invisible line, Wuldon begins to shake himself violently, spraying water everywhere."
    ""

translate tranditional_chinese Chelforte_Vurro_efbd22af:

    # w "Good as new."
    w ""

translate tranditional_chinese Chelforte_Vurro_02140273:

    # "The Werewolf takes this opportunity to put down his pack and take out a pickaxe."
    ""

translate tranditional_chinese Chelforte_Vurro_7fdacbf9:

    # w "Let's get going."
    w ""

translate tranditional_chinese Chelforte_Vurro_5e5915d7:

    # "The brown werewolf is turning around with a small boulder in his arms when he spots you."
    ""

translate tranditional_chinese Chelforte_Vurro_0d3fc131:

    # v "Hello [e]. Anything I can help you with?"
    v ""

translate tranditional_chinese Chelforte_Vurro_74fe9d3d:

    # e "No, I'm doing well enough on my end. I was actually wondering if you wanted any help?"
    e ""

translate tranditional_chinese Chelforte_Vurro_e9deefcb:

    # "Vurro gives you a small grin as he drops the boulder over in a growing pile to his left."
    ""

translate tranditional_chinese Chelforte_Vurro_c2523471:

    # v "I think I have things well in hand over here, but if you want to help, be my guest."
    v ""

translate tranditional_chinese Chelforte_Vurro_62a32bc9:

    # e "Alright, how should I go about this then?"
    e ""

translate tranditional_chinese Chelforte_Vurro_8ca10b84:

    # v "Just pick up rocks from that pile over there, and drag them over to the corner."
    v ""

translate tranditional_chinese Chelforte_Vurro_0a4ce564:

    # e "Sounds good. I'll stop when I need to take a break."
    e ""

translate tranditional_chinese Chelforte_Vurro_e19250f6:

    # v "Absolutely. We still need you exploring after all!"
    v ""

translate tranditional_chinese Chelforte_Vurro_e8bd2563:

    # "With that said, the two of you begin hauling rocks together, sharing small talk and laughing all the while."
    ""

translate tranditional_chinese Chelforte_Vurro_82889b60:

    # msg "Your time hauling rocks with Vurro has dealt 100 HP damage to you."
    msg ""

translate tranditional_chinese Chelforte_Vurro_4efedb87:

    # msg "Despite that, you feel the aching in your muscles sooth slightly as you drink from the cave's water. You gained 100 EXP."
    msg ""

translate tranditional_chinese Chelforte_Vurro_91f1d522:

    # msg "You are now Level [pc.level]! Check your inventory to allocate your level points."
    msg ""

translate tranditional_chinese Chelforte_Vurro_506532cb:

    # "You pick up a slate rock."
    ""

translate tranditional_chinese Chelforte_Vurro_77eb8817:

    # e "I would, but I need to conserve my energy for exploring."
    e ""

translate tranditional_chinese Chelforte_Vurro_c6174b6c:

    # v "That's alright! Thanks for checking in!"
    v ""

translate tranditional_chinese Chelforte_Vurro_e1a361f3:

    # "The smiling werewolf turns around and begins hauling rocks once more."
    ""

translate tranditional_chinese Chelforte_Vurro_cd962345:

    # "Maybe you shouldn't bother him again unless you want to help."
    ""

translate tranditional_chinese Chelforte_Vurro_7f70bbca:

    # "Vurro is wiping sweat off of his brow when you find him again."
    ""

translate tranditional_chinese Chelforte_Vurro_84ff3a7d:

    # v "You here to help?"
    v ""

translate tranditional_chinese Chelforte_Vurro_55992b1f:

    # e "I'm a bit bored of exploring, so I think I'll hop on if you don't mind."
    e ""

translate tranditional_chinese Chelforte_Vurro_18c3dfd7:

    # v "Wait... you're too out of shape for such a heavy duty."
    v ""

translate tranditional_chinese Chelforte_Vurro_0dbf2382:

    # e "Am I?"
    e ""

translate tranditional_chinese Chelforte_Vurro_8228c5e2:

    # "All you get is a smile from Vurro, as he begins hauling rocks once more."
    ""

translate tranditional_chinese Chelforte_Vurro_9fdc718a:

    # "Seeing that Vurro understandably does not want to waste time, you join him at once."
    ""

translate tranditional_chinese Chelforte_Vurro_a2622cda:

    # "The two of you spend your time chatting and teasing, occasionally receiving a pebble to the back of the head by an annoyed Wuldon."
    ""

translate tranditional_chinese Chelforte_Vurro_e8bd2563_1:

    # "With that said, the two of you begin hauling rocks together, sharing small talk and laughing all the while."
    ""

translate tranditional_chinese Chelforte_Vurro_159b8d4b:

    # msg "Your time hauling rocks with Vurro has dealt 100 HP damage to you, none of which is from Wuldon."
    msg ""

translate tranditional_chinese Chelforte_Vurro_5bc159ba:

    # msg "As you drink from the cave's water, you feel the aching of your muscles subside, torn muscle repairing instantly without hindering its growth. You gained 100 EXP."
    msg ""

translate tranditional_chinese Chelforte_Vurro_91f1d522_1:

    # msg "You are now Level [pc.level]! Check your inventory to allocate your level points."
    msg ""

translate tranditional_chinese Chelforte_Vurro_506532cb_1:

    # "You pick up a slate rock."
    ""

translate tranditional_chinese Chelforte_Vurro_e7a0226f:

    # e "Sorry, I don't think I can really afford to right now."
    e ""

translate tranditional_chinese Chelforte_Vurro_8ab8d3d9:

    # "A small grin from the werewolf tells you he doesn't mind."
    ""

translate tranditional_chinese Chelforte_Vurro_b6d4813e:

    # v "Alright, but make sure to come here and tell me if anything cool happens, alright?"
    v ""

translate tranditional_chinese Chelforte_Vurro_1bfe657f:

    # "He pauses for a moment."
    ""

translate tranditional_chinese Chelforte_Vurro_5dfb2795:

    # v "Or scream if you're in trouble."
    v ""

translate tranditional_chinese Chelforte_Vurro_bd5b0267:

    # "Deciding his break is over, Vurro begins hauling rocks once more."
    ""

translate tranditional_chinese Chelforte_Cavern_Ward_a30ae46e:

    # "You look at the pulsating rock in front of you..."
    ""

translate tranditional_chinese Chelforte_Cavern_Ward_63309e96:

    # "It's weird that the rock activated by the flowing water is seen here also..."
    ""

translate tranditional_chinese Chelforte_Cavern_Ward_53976138:

    # "You shake your head."
    ""

translate tranditional_chinese Chelforte_Cavern_Ore1_1fdf53f3:

    # "You stare at the ore vein, there seems to be some minerals inside the wall..."
    ""

translate tranditional_chinese Chelforte_Cavern_Ore1_f0ce74f4:

    # "However, you do not have the tool to extract the ore..."
    ""

translate tranditional_chinese Chelforte_Cavern_Ore1_d9888168:

    # "You take out your pickaxe and start striking it against the ore."
    ""

translate tranditional_chinese Chelforte_Cavern_Ore1_a5c03a12:

    # "After a long and grueling moments, you finally obtains an iron ore."
    ""

translate tranditional_chinese Chelforte_Cavern_Ore1_4eb2a229:

    # "There's nothing for you to mine for now..."
    ""

translate tranditional_chinese Chelforte_Cavern_Ore1_a08666a7:

    # "Maybe the ore will replenish... eventually."
    ""

translate tranditional_chinese Chelforte_Cavern_Ore2_1fdf53f3:

    # "You stare at the ore vein, there seems to be some minerals inside the wall..."
    ""

translate tranditional_chinese Chelforte_Cavern_Ore2_f0ce74f4:

    # "However, you do not have the tool to extract the ore..."
    ""

translate tranditional_chinese Chelforte_Cavern_Ore2_d9888168:

    # "You take out your pickaxe and start striking it against the ore."
    ""

translate tranditional_chinese Chelforte_Cavern_Ore2_684a288a:

    # "After a long and grueling moments, you finally obtains a lodestone ore."
    ""

translate tranditional_chinese Chelforte_Cavern_Ore2_4eb2a229:

    # "There's nothing for you to mine for now..."
    ""

translate tranditional_chinese Chelforte_Cavern_Ore2_a08666a7:

    # "Maybe the ore will replenish... eventually."
    ""

translate tranditional_chinese Chelforte_Cavern_Ore3_1fdf53f3:

    # "You stare at the ore vein, there seems to be some minerals inside the wall..."
    ""

translate tranditional_chinese Chelforte_Cavern_Ore3_f0ce74f4:

    # "However, you do not have the tool to extract the ore..."
    ""

translate tranditional_chinese Chelforte_Cavern_Ore3_d9888168:

    # "You take out your pickaxe and start striking it against the ore."
    ""

translate tranditional_chinese Chelforte_Cavern_Ore3_017e7293:

    # "After a long and grueling moments, you finally obtains a copper ore."
    ""

translate tranditional_chinese Chelforte_Cavern_Ore3_4eb2a229:

    # "There's nothing for you to mine for now..."
    ""

translate tranditional_chinese Chelforte_Cavern_Ore3_a08666a7:

    # "Maybe the ore will replenish... eventually."
    ""

translate tranditional_chinese Chelforte_Cavern_Ore3_c7b6d28a:

    # "Your health lowers by 10 HP."
    ""

translate tranditional_chinese Cavern_Entrance_Loop_1d7ad46b:

    # "You look at the sign... it says Chelforte Cavern. With a drawing of a huge werewolf and rocks from the cave..."
    ""

translate tranditional_chinese Cavern_Entrance_Loop_697e01bc:

    # "It seems to signal danger ahead."
    ""

translate tranditional_chinese Cavern_Entrance_Loop_fdd0f5a9:

    # "Shaking your head, you enter the cavern with caution..."
    ""

translate tranditional_chinese Cavern_Entrance_Loop_784896c6:

    # "You notice a small creature right inside the bush... it seem to be craving for a plant inside your backpack..."
    ""

translate tranditional_chinese Cavern_Entrance_Loop_a599dd03:

    # "However... it seems you don't know how to lure it out... maybe you need to learn from a certain person..."
    ""

translate tranditional_chinese Cavern_Entrance_Loop_242191df:

    # "You give it a piece..."
    ""

translate tranditional_chinese Cavern_Entrance_Loop_20d69762:

    # "It seems it doesn't appreciate the gift, it snatches the piece right out of your hand."
    ""

translate tranditional_chinese Cavern_Entrance_Loop_a20cefa7:

    # "..."
    ""

translate tranditional_chinese Cavern_Entrance_Loop_95226dee:

    # "You lost a piece of Hemp, and the creature is still inside."
    ""

translate tranditional_chinese Cavern_Entrance_Loop_242191df_1:

    # "You give it a piece..."
    ""

translate tranditional_chinese Cavern_Entrance_Loop_20d69762_1:

    # "It seems it doesn't appreciate the gift, it snatches the piece right out of your hand."
    ""

translate tranditional_chinese Cavern_Entrance_Loop_a20cefa7_1:

    # "..."
    ""

translate tranditional_chinese Cavern_Entrance_Loop_c4467c2e:

    # "You lost a piece of Ginger, and the creature is still inside."
    ""

translate tranditional_chinese Cavern_Entrance_Loop_242191df_2:

    # "You give it a piece..."
    ""

translate tranditional_chinese Cavern_Entrance_Loop_20d69762_2:

    # "It seems it doesn't appreciate the gift, it snatches the piece right out of your hand."
    ""

translate tranditional_chinese Cavern_Entrance_Loop_a20cefa7_2:

    # "..."
    ""

translate tranditional_chinese Cavern_Entrance_Loop_efa64d93:

    # "You lost a piece of Reed, and the creature is still inside."
    ""

translate tranditional_chinese Cavern_Entrance_Loop_242191df_3:

    # "You give it a piece..."
    ""

translate tranditional_chinese Cavern_Entrance_Loop_20d69762_3:

    # "It seems it doesn't appreciate the gift, it snatches the piece right out of your hand."
    ""

translate tranditional_chinese Cavern_Entrance_Loop_a20cefa7_3:

    # "..."
    ""

translate tranditional_chinese Cavern_Entrance_Loop_173b8db3:

    # "You lost a piece of Rosemary, and the creature is still inside."
    ""

translate tranditional_chinese Cavern_Entrance_Loop_242191df_4:

    # "You give it a piece..."
    ""

translate tranditional_chinese Cavern_Entrance_Loop_8a4ca899:

    # "It takes the carrot quickly back into the bush.... and returned with a weird flower..."
    ""

translate tranditional_chinese Cavern_Entrance_Loop_79ba4270:

    # "You can instantly feel the effect of the flower when it touches your fur, you carefully store it in your bag."
    ""

translate tranditional_chinese Cavern_Entrance_Loop_72e5ad9a:

    # "You thank the little creature, it blinks at you and retreats back into the bush."
    ""

translate tranditional_chinese Cavern_Entrance_Loop_8a4ca899_1:

    # "It takes the carrot quickly back into the bush.... and returned with a weird flower..."
    ""

translate tranditional_chinese Cavern_Entrance_Loop_410d02b0:

    # "When you look closely, you realise it's just a chrysanthemum."
    ""

translate tranditional_chinese Cavern_Entrance_Loop_72e5ad9a_1:

    # "You thank the little creature, it blinks at you and retreats back into the bush."
    ""

translate tranditional_chinese Cavern_Entrance_Loop_242191df_5:

    # "You give it a piece..."
    ""

translate tranditional_chinese Cavern_Entrance_Loop_4d9ad675:

    # "It takes the carrot quickly back into the bush.... but it doesn't return you anything..."
    ""

translate tranditional_chinese Cavern_Entrance_Loop_a20cefa7_4:

    # "..."
    ""

translate tranditional_chinese Cavern_Entrance_Loop_7df2caa2:

    # "Maybe.... you need a higher {color=#d1e431}Charisma{/color} for the little creature, or try again."
    ""

translate tranditional_chinese Cavern_Entrance_Loop_242191df_6:

    # "You give it a piece..."
    ""

translate tranditional_chinese Cavern_Entrance_Loop_20d69762_4:

    # "It seems it doesn't appreciate the gift, it snatches the piece right out of your hand."
    ""

translate tranditional_chinese Cavern_Entrance_Loop_a20cefa7_5:

    # "..."
    ""

translate tranditional_chinese Cavern_Entrance_Loop_78a61ade:

    # "You lost a piece of Barley, and the creature is still inside."
    ""

translate tranditional_chinese Cavern_Entrance_Loop_242191df_7:

    # "You give it a piece..."
    ""

translate tranditional_chinese Cavern_Entrance_Loop_20d69762_5:

    # "It seems it doesn't appreciate the gift, it snatches the piece right out of your hand."
    ""

translate tranditional_chinese Cavern_Entrance_Loop_a20cefa7_6:

    # "..."
    ""

translate tranditional_chinese Cavern_Entrance_Loop_358fc31b:

    # "You lost a piece of Chrysanthemum, and the creature is still inside."
    ""

translate tranditional_chinese Cavern_Entrance_Loop_242191df_8:

    # "You give it a piece..."
    ""

translate tranditional_chinese Cavern_Entrance_Loop_20d69762_6:

    # "It seems it doesn't appreciate the gift, it snatches the piece right out of your hand."
    ""

translate tranditional_chinese Cavern_Entrance_Loop_a20cefa7_7:

    # "..."
    ""

translate tranditional_chinese Cavern_Entrance_Loop_983d4d28:

    # "You lost a piece of Herb of Grace, and the creature is still inside."
    ""

translate tranditional_chinese Cavern_Entrance_Loop_2dd49d8c:

    # msg "Do you want to leave the area, states of the dungeon will not be saved." nointeract
    msg "" nointeract

translate tranditional_chinese strings:

    old "Do you want to pick it up?"
    new ""

    old "Yes{#pickupcarrots}"
    new ""

    old "No{#pickupcarrots}"
    new ""

    old "Yes{#leavesplittrail}"
    new ""

    old "No{#leavesplittrail}"
    new ""

    old "Do you wish to reset the placement of barrels?"
    new ""

    old "Yes{#resetsplittrailpuzzle}"
    new ""

    old "No{#resetsplittrailpuzzle}"
    new ""

    old "Do you want to carry the flower...? It might wither after a while in your hand."
    new ""

    old "Yes{#tulipcarry}"
    new ""

    old "No{#tulipcarry}"
    new ""

    old "Do you wish to put the tulip in the pot?"
    new ""

    old "Yes{#tulippotpuzzle}"
    new ""

    old "No{#tulippotpuzzle}"
    new ""

    old "Yes{#pickupcarrotbutcaproot}"
    new ""

    old "No{#pickupcarrotbutcaproot}"
    new ""

    old "Do you want to go through the hole?"
    new ""

    old "Yes{#chelfortehole}"
    new ""

    old "No{#chelfortehole}"
    new ""

    old "What should you do?"
    new ""

    old "Go through the hole"
    new ""

    old "Yes{#leavechelforte}"
    new ""

    old "No{#leavechelforte}"
    new ""

    old "Accept Wuldon's Help?"
    new ""

    old "Yes{#letwuldonhelp}"
    new ""

    old "No{#letwuldonhelp}"
    new ""

    old "Explore the Cave"
    new ""

    old "Do you want to help Vurro?"
    new ""

    old "Yes{#helpvurro}"
    new ""

    old "No{#helpvurro}"
    new ""

    old "Yes{#helpvurro2}"
    new ""

    old "No{#helpvurro2}"
    new ""

    old "Do you want to fight the stone ward?"
    new ""

    old "Yes{#fightchelfortestoneward}"
    new ""

    old "No{#fightchelfortestoneward}"
    new ""

    old "What do you wish to use...?"
    new ""

    old "Hemp"
    new ""

    old "Ginger"
    new ""

    old "Reed"
    new ""

    old "Rosemary"
    new ""

    old "Carrot"
    new ""

    old "Barley"
    new ""

    old "Chrysanthemum"
    new ""

    old "Herb of Grace"
    new ""

    old "Yes{#leavecavernentrance}"
    new ""

    old "No{#leavecavernentrance}"
    new ""


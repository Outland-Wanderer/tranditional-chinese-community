translate tranditional_chinese battle_attack_script_a0b503c2:

    # "Amble strikes his hammer against the fluffy leaves, but it bounces off the bridgeroot slickly."
    ""

translate tranditional_chinese battle_attack_script_cbc9cf6d:

    # "You raise your fist and punch as hard as you can into the [target.name!t]. but it swiftly dodges away from your blow."
    ""

translate tranditional_chinese battle_attack_script_69cc0745:

    # "You raise your sword and slash at the monster, but the [target.name!t] quickly dodges your attack."
    ""

translate tranditional_chinese battle_attack_script_0f1c7eb7:

    # "You slam your axe into the monster, but the [target.name!t] quickly dodges your attack."
    ""

translate tranditional_chinese battle_attack_script_d5018a91:

    # "You hold and shoot an arrow at the slime, but it narrowly misses the [target.name!t]."
    ""

translate tranditional_chinese battle_attack_script_2ce302db:

    # "You hold your fist and punch as hard as you can into the [target.name!t]. but you punch the wall instead. Hopefully no one saw that..."
    ""

translate tranditional_chinese battle_attack_script_d0065919:

    # "Your swing your sword at the [target.name!t], but it hits the wall instead."
    ""

translate tranditional_chinese battle_attack_script_3000ebcd:

    # "You slam your axe into the [target.name!t], but it hits the wall instead."
    ""

translate tranditional_chinese battle_attack_script_bfe0fafc:

    # "You bring up your bow and aim. You release the bowstring, and it barely miss the [target.name!t]."
    ""

translate tranditional_chinese battle_attack_script_4b22c62f:

    # "You slash your [pc.weapon.name!t] at the [enemymy!t]'s arm, but he leaps back, avoiding the blow by inches."
    ""

translate tranditional_chinese battle_attack_script_3e964837:

    # "You try to slam your [pc.weapon.name!t] into the [enemymy!t]'s head, but he leaps back and avoids the blow by mere inches."
    ""

translate tranditional_chinese battle_attack_script_dee53981:

    # "You aim and shoot your [pc.weapon.name!t] at the [enemymy!t], but he leaps back, avoiding the arrow by mere inches."
    ""

translate tranditional_chinese battle_attack_script_7f34fdd5:

    # "You throw your fist at the [enemymy!t], but he leaps back, the blow only serving to ruffle his fur a little."
    ""

translate tranditional_chinese battle_attack_script_309a9672:

    # gt "You foul varmint, you really think your little trick is going to hurt me? Now, taste my spear!"
    gt ""

translate tranditional_chinese battle_attack_script_45dfa330:

    # gt "T-that wasn't a good attempt, you need to do better."
    gt ""

translate tranditional_chinese battle_attack_script_d7edc6aa:

    # bearGuard "What a waste of time, let me show you how a real snow warrior fight."
    bearGuard ""

translate tranditional_chinese battle_attack_script_0becd5f1:

    # "You slash your [pc.weapon.name!t] at the [target.name!t], but it strikes into nothing but a few stranded leaves and moss."
    ""

translate tranditional_chinese battle_attack_script_86e89ad0:

    # "You slam your [pc.weapon.name!t] at the [target.name!t], but it strikes into nothing but a few stranded leaves and moss."
    ""

translate tranditional_chinese battle_attack_script_046a3c9e:

    # "You aim and shoot your [pc.weapon.name!t] at the [target.name!t], but it strikes into nothing but a few stranded leaves and moss."
    ""

translate tranditional_chinese battle_attack_script_70f3831c:

    # "You hold your fist, with all your might you throw a punch at the [target.name!t], but it strikes into nothing but a few stranded leaves and moss."
    ""

translate tranditional_chinese battle_attack_script_e1ac7fa2:

    # "The [target.name!t] stands there, still emotionless, he doesn't seem to notice your feeble attack, not even the slightest."
    ""

translate tranditional_chinese battle_attack_script_00a3dd32:

    # "You slash your [pc.weapon.name!t] at the [target.name!t], but it turns ephereal and dodges swiftly in the air."
    ""

translate tranditional_chinese battle_attack_script_b3c00be1:

    # yu "Shit, my sword's not working..."
    yu ""

translate tranditional_chinese battle_attack_script_754b49b3:

    # "The [target.name!t] floats motionless in face of your attack."
    ""

translate tranditional_chinese battle_attack_script_d8a1e631:

    # "You slash your [pc.weapon.name!t] at the arms of the [target.name!t], it slides right off his fluffy arm, while you look back in disbelief."
    ""

translate tranditional_chinese battle_attack_script_bce5b210:

    # "You slam your [pc.weapon.name!t] at the arms of the [target.name!t], it slides right off his fluffy arm, while you look back in disbelief."
    ""

translate tranditional_chinese battle_attack_script_9857982e:

    # "You aim and shoot your [pc.weapon.name!t] at the arms of the [target.name!t], it pierces right through his soft fur without touching his skin, leaving you frozen in disbelief."
    ""

translate tranditional_chinese battle_attack_script_dba94e4a:

    # "You hold your fist and throw it at the [target.name!t], but it hits nothing and leaves you standing instead."
    ""

translate tranditional_chinese battle_attack_script_48ed0abb:

    # ww "Unlucky. But not surprised, hmmm..."
    ww ""

translate tranditional_chinese battle_attack_script_1b735359:

    # "The [target.name!t] laughs loudly while flexing his claws, it almost seems he's scoffing at your attempt of attack."
    ""

translate tranditional_chinese battle_attack_script_3d02284b:

    # "You press on, trying to land a hit, but Vurro continues to evade your blows."
    ""

translate tranditional_chinese battle_attack_script_2c02f8d7:

    # "Despite his illness, Vurro moves with surprising grace, ducking and weaving out of your way."
    ""

translate tranditional_chinese battle_attack_script_f3cf8947:

    # v "Hey, aim better. I'm walking only slightly faster than a training dummy."
    v ""

translate tranditional_chinese battle_attack_script_a040d9c4:

    # v "Not saying Uffe is fast and small enough to dodge your blow, but you can't swing your fist anywhere hoping it hits something."
    v ""

translate tranditional_chinese battle_attack_script_dbd57fbe:

    # "You slash your [pc.weapon.name!t] at the [target.name!t], but he hops away in time, dodging from your attack."
    ""

translate tranditional_chinese battle_attack_script_464c0cbb:

    # "You slam your [pc.weapon.name!t] at the [target.name!t], but he hops away in time, dodging from your attack."
    ""

translate tranditional_chinese battle_attack_script_1c09c114:

    # "You aim and shoot your [pc.weapon.name!t] at the [target.name!t], but he hops away in time, dodging from your attack."
    ""

translate tranditional_chinese battle_attack_script_2719255f:

    # "You hold your fist and throw it at the [target.name!t], but he hops away in time, dodging from your attack."
    ""

translate tranditional_chinese battle_attack_script_9ff75fbd:

    # bd "Come on, try harder! You'll never defeat me with such sloppy attacks!"
    bd ""

translate tranditional_chinese battle_attack_script_dca0d60e:

    # bd "Swing and a miss! You'll have to do better than that!"
    bd ""

translate tranditional_chinese battle_attack_script_911caff7:

    # gnl "You swing, I dance!"
    gnl ""

translate tranditional_chinese battle_attack_script_619312a4:

    # gnl "Jump, spin, you miss!"
    gnl ""

translate tranditional_chinese battle_attack_script_0cbff535:

    # "The [target.name!t] laughs loudly while crouching on the grass, it almost seems he's scoffing at your attempt of attack."
    ""

translate tranditional_chinese battle_attack_script_6430a4d5:

    # "Amble strikes his hammer against the fluffy leaves of the bridgeroot, it hits his core very effectively as the green monster staggers a bit backwards, you can clearly see the crater his hammer creates there."
    ""

translate tranditional_chinese battle_attack_script_ac32d1b0:

    # "You hold your fist and punch as hard as you can into the [target.name!t]. It is as disgusting as it is effective."
    ""

translate tranditional_chinese battle_attack_script_4fe140e4:

    # "You growl and swing your sword at the monster, slashing right through the [target.name!t]."
    ""

translate tranditional_chinese battle_attack_script_8ed4f646:

    # "You growl and swing your axe at the monster, slashing right through the [target.name!t]."
    ""

translate tranditional_chinese battle_attack_script_04385294:

    # "You hold your bow and aim at the monster, you release the arrow and it flies true, hitting the [target.name!t] right in its core."
    ""

translate tranditional_chinese battle_attack_script_3a5b1398:

    # "You raise your fist and punch as hard as you can into the dummy."
    ""

translate tranditional_chinese battle_attack_script_d99a2c47:

    # "You growl and swing your sword, slashing right into the dummy."
    ""

translate tranditional_chinese battle_attack_script_285e2b00:

    # "You growl and swing your axe, slashing right into the dummy."
    ""

translate tranditional_chinese battle_attack_script_571e5fff:

    # "You bring up your bow and aim. The arrow flies right into its bull's eye."
    ""

translate tranditional_chinese battle_attack_script_9003ef3c:

    # "You try to slice your [pc.weapon.name!t] into the [enemymy!t]'s arm. He brings his spear up to block, but fails to stop the strike entirely."
    ""

translate tranditional_chinese battle_attack_script_f1d3deb2:

    # "You feint an overhead chop with your [pc.weapon.name!t], before bringing up your foot for a kick to his chest as he tries to block, knocking him down on the ground."
    ""

translate tranditional_chinese battle_attack_script_a6356e7c:

    # "You slash your [pc.weapon.name!t] at the [enemymy!t] in a wide, horizontal arc. The [enemymy!t] leaps back out of range, but not before the blade nicks a shallow cut across his stomach."
    ""

translate tranditional_chinese battle_attack_script_5b9317f9:

    # "You slam the butt of your [pc.weapon.name!t] into the [enemymy!t]'s head, knocking him on the ground."
    ""

translate tranditional_chinese battle_attack_script_8f29c744:

    # "You aim and shoot your [pc.weapon.name!t] at the [enemymy!t]. The arrow hits him right in the shoulder. "
    ""

translate tranditional_chinese battle_attack_script_d66a0002:

    # "You run while shooting your [pc.weapon.name!t] at the [enemymy!t], knocking him to the ground. He growls at you before getting up, albeit disheveled."
    ""

translate tranditional_chinese battle_attack_script_9db3d034:

    # "You throw your fist at the [enemymy!t], hitting him right across the face. The sheer impact knocks him to the ground."
    ""

translate tranditional_chinese battle_attack_script_859ecc10:

    # "You slam your fist into the [enemymy!t]'s stomach. As he doubles over in pain, you kick out his legs under him, knocking him flat on the ground."
    ""

translate tranditional_chinese battle_attack_script_d21654c5:

    # "A bright red gash appearing in his arm."
    ""

translate tranditional_chinese battle_attack_script_3f10a603:

    # "He growls at you before getting up, albeit disheveled."
    ""

translate tranditional_chinese battle_attack_script_3b02969e:

    # "Drops of blood spill down the front of his body."
    ""

translate tranditional_chinese battle_attack_script_18302ba4:

    # "It seems you've hit the goat critically, dealing [oa[4]] HP!"
    ""

translate tranditional_chinese battle_attack_script_7bbbc409:

    # "You slash your [pc.weapon.name!t] at the [target.name!t], as your blade grazes through the leaves and moss on [target.name!t], and a chunk of leaves falls off of him, casually."
    ""

translate tranditional_chinese battle_attack_script_c0e19e1a:

    # "You slam your [pc.weapon.name!t] at the [target.name!t], as it strikes through the leaves and moss on [target.name!t], and a chunk of leaves falls off of him, casually."
    ""

translate tranditional_chinese battle_attack_script_2c722d6b:

    # "You aim and shoot your [pc.weapon.name!t] at the [target.name!t], as your arrow grazes through the leaves and moss on [target.name!t], and a chunk of leaves falls off of him, casually."
    ""

translate tranditional_chinese battle_attack_script_3aa0091a:

    # "You hold your fist, with all your might you throw a punch at the [target.name!t], as your blade grazes through the leaves and moss on [target.name!t], and a chunk of leaves falls off of him, casually."
    ""

translate tranditional_chinese battle_attack_script_3bb039f4:

    # "The [target.name!t] grunts loudly, seems more than mildly annoyed now that you're hurting him."
    ""

translate tranditional_chinese battle_attack_script_422c861a:

    # "You slash your [pc.weapon.name!t] at the [target.name!t], your blade grazes through the ghostly form of the [target.name!t]."
    ""

translate tranditional_chinese battle_attack_script_3e8d701f:

    # "The [target.name!t] vibrates, seems more than mildly annoyed now that you're hurting him."
    ""

translate tranditional_chinese battle_attack_script_1a6ba155:

    # "You unleash a flurry of punches, each strike aimed at Vurro's chest and shoulder."
    ""

translate tranditional_chinese battle_attack_script_7ba4baa4:

    # "Momentarily caught off-guard, Vurro tries to evade your attack, but you manage to graze his side with a swift punch."
    ""

translate tranditional_chinese battle_attack_script_6ee8359c:

    # "You feint to the right before landing a solid punch to Vurro's shoulder."
    ""

translate tranditional_chinese battle_attack_script_55e10f3e:

    # "It's a clean hit, and you can see a flicker of surprise in Vurro's eyes as he stumbles back a step."
    ""

translate tranditional_chinese battle_attack_script_c17e6bad:

    # "You throw a punch before executing a quick spin, delivering a swift kick to Vurro's midsection."
    ""

translate tranditional_chinese battle_attack_script_9ffe5e36:

    # "The kick lands, and Vurro grunts in surprise as he stumbles back, momentarily winded."
    ""

translate tranditional_chinese battle_attack_script_12680f06:

    # "You slash your [pc.weapon.name!t] at the [target.name!t], your blade grazes through the [target.name!t]'s stomach. Drops of blood drips through his body."
    ""

translate tranditional_chinese battle_attack_script_151b9439:

    # "You slash your [pc.weapon.name!t] at the [target.name!t], knocking him on the ground. He growls at you before getting up, albeit disheveled."
    ""

translate tranditional_chinese battle_attack_script_e5a49a17:

    # "You slam your [pc.weapon.name!t] at the [target.name!t], your blade grazes through the [target.name!t]'s stomach. Drops of blood drips through his body."
    ""

translate tranditional_chinese battle_attack_script_3d3f0282:

    # "You slam your [pc.weapon.name!t] at the [target.name!t], knocking him on the ground. He growls at you before getting up, albeit disheveled."
    ""

translate tranditional_chinese battle_attack_script_3aed5a1a:

    # "You aim and shoot your [pc.weapon.name!t] at the [target.name!t], the arrow hit right into his shoulder and he screams in agony."
    ""

translate tranditional_chinese battle_attack_script_e4ccce4c:

    # "You run while shooting your [pc.weapon.name!t] at the [target.name!t], knocking him on the ground. He growls at you before getting up, albeit disheveled."
    ""

translate tranditional_chinese battle_attack_script_a0f9e82d:

    # "You throw your fist at the [target.name!t], hitting him right across his face, the sheer impact knocks him on the ground."
    ""

translate tranditional_chinese battle_attack_script_f237f06f:

    # "You punch into the [target.name!t]'s stomach, grabbing him and slam him on the ground hard."
    ""

translate tranditional_chinese battle_attack_script_24bcc736:

    # "His health decreases by [oa[4]] HP."
    ""

translate tranditional_chinese battle_attack_script_6b4f73d0:

    # "Critical hit! [target.name!t]'s health now decreases by [oa[4]] HP!"
    ""

translate tranditional_chinese battle_attack_script_8d723584:

    # v "Ouch, your fist does hurt. Wuldon's gonna have a hard time pissing you off, doesn't he?"
    v ""

translate tranditional_chinese battle_attack_script_7d3cd5bc:

    # v "That was quite impressive for a non-werewolf. You do have some tricks up your sleeves."
    v ""

translate tranditional_chinese battle_attack_script_cf32bdd4:

    # ww "Come here... little prey, stop resisting."
    ww ""

translate tranditional_chinese battle_attack_script_821b8d28:

    # ww "Huh... This prey is definitely moving. And I'll prefer a moving one when I get a hold of you."
    ww ""

translate tranditional_chinese battle_attack_script_8ffa9b69:

    # "The [target.name!t] howls in anger, you can feel the imminent danger as other werewolves in the forest respond to his howl."
    ""

translate tranditional_chinese battle_attack_script_4e97572b:

    # gt "Aggghh.... W-what the... You little furry lizard, I'll strike you down without mercy!"
    gt ""

translate tranditional_chinese battle_attack_script_8787f4c7:

    # gt "Grrrrr! L-lucky hit... Let me teach you how to fight properly!"
    gt ""

translate tranditional_chinese battle_attack_script_8787f4c7_1:

    # gt "Grrrrr! L-lucky hit... Let me teach you how to fight properly!"
    gt ""

translate tranditional_chinese battle_attack_script_d3d10638:

    # bearGuard "Hnnngh! Just a scratch, you'll learn to regret even touching me."
    bearGuard ""

translate tranditional_chinese battle_attack_script_ec2ed84a:

    # bearGuard "Arrgh! By Ookko's bless, I- I won't let you g-get away with this."
    bearGuard ""

translate tranditional_chinese battle_attack_script_c5074ad8:

    # gnl "O-Ouch! Fingertips graze, almost!"
    gnl ""

translate tranditional_chinese battle_attack_script_3c0044de:

    # gnl "Y-your blow, it finds me!"
    gnl ""

translate tranditional_chinese battle_attack_script_ef32e0d1:

    # bd "Not bad, not bad at all. But I'm not done with you yet!"
    bd ""

translate tranditional_chinese battle_attack_script_9e05dcb6:

    # bd "Alright, this is gonna worth my gold so much."
    bd ""

translate tranditional_chinese battle_attack_script_e1c89fe5:

    # ww "Argh...! You can really pack a punch do you not? Can't wait to pin you down and get a taste of your flesh."
    ww ""

translate tranditional_chinese battle_attack_script_4be9928d:

    # ww "Hnnngh!!! Now I'm getting real angry. And you don't want to see me when I'm angry, little prey."
    ww ""

translate tranditional_chinese battle_attack_script_29d84905:

    # "You can barely hear barking sound of the [target.name!t], it seems like he doesn't appreciate a trespasser, and you've angered him further."
    ""

translate tranditional_chinese battle_attack_script_a3b94c0e:

    # gt "Ummmph.. How... I-I can't. Chief, please give me the strength to defeat this insolent whelp!"
    gt ""

translate tranditional_chinese battle_attack_script_736253fc:

    # gt "Damn... didn't know a courier can hit that hard...!"
    gt ""

translate tranditional_chinese battle_attack_script_74a6a844:

    # gt "Hnnnngh... M-my brothers, they will not spare your pathetic life. T-they're coming... any second now. Y-you better run."
    gt ""

translate tranditional_chinese battle_attack_script_b9b79704:

    # gt "Damn... didn't know a courier can hit that hard...! Give me all you got!"
    gt ""

translate tranditional_chinese battle_attack_script_293c30bb:

    # bearGuard "Ngggg... I'll fight until my last breath, b-because I'm a warrior!"
    bearGuard ""

translate tranditional_chinese battle_attack_script_82fbad5d:

    # bearGuard "T'is but a test of my strength, I'll s-strike you down, w-watch it."
    bearGuard ""

translate tranditional_chinese battle_attack_script_f1fbe73b:

    # gnl "Gnoll feels that, argh!"
    gnl ""

translate tranditional_chinese battle_attack_script_be54626b:

    # gnl "Y-ou strong... I-I see."
    gnl ""

translate tranditional_chinese battle_attack_script_c678487d:

    # bd "Argh! Lucky shot! But don't get cocky, I've got plenty more fight in me!"
    bd ""

translate tranditional_chinese battle_attack_script_69eb351b:

    # bd "Hey, let's talk this out, huh? I'll make it worth your while!"
    bd ""

translate tranditional_chinese battle_flirt_script_2f33da57:

    # "You caress your body in front of the dummy, calling its name in some silly fashion."
    ""

translate tranditional_chinese battle_flirt_script_7843803c:

    # "Getting closer to the motionless dummy, you try to get a reaction from it by touching it in the groin area... but it doesn't seem to react to your advance."
    ""

translate tranditional_chinese battle_flirt_script_a4b1ddae:

    # "Flirt doesn't sound like a great idea on these creatures..."
    ""

translate tranditional_chinese battle_flirt_script_021d1316:

    # "As much as you try to... flirt with the slime, it doesn't flinch, or get aroused."
    ""

translate tranditional_chinese battle_flirt_script_6bcff645:

    # "You back off before it tries to grab a hold of your body."
    ""

translate tranditional_chinese battle_flirt_script_fdf9de27:

    # "You turn around and rub your hand all over your own burly cheeks, feeling and brushing against your ass while you shake your hip."
    ""

translate tranditional_chinese battle_flirt_script_f8c9ea12:

    # "You scrape your member lightly, running your claw from your inner thigh to the back of your balls, you tug at it tightly while staring at [target.name!t] seductively."
    ""

translate tranditional_chinese battle_flirt_script_38976fda:

    # "You cup at your fluffy chest, drawing circles around the area of your nipples. You smile at the [target.name!t] while your chest bounce up and down slightly."
    ""

translate tranditional_chinese battle_flirt_script_7e59b417:

    # v "I'm not sure if this is part of the training, but I don't think you're going to survive after showing Uffe this, so don't do that."
    v ""

translate tranditional_chinese battle_flirt_script_f0115095:

    # "Vurro shoots you a playful smile, mildly amused at your demonstration."
    ""

translate tranditional_chinese battle_flirt_script_c2ea7413:

    # "It doesn't seem to have noticed, or have the capability to even do that."
    ""

translate tranditional_chinese battle_flirt_script_7aae6f2c:

    # "You continue your act for about a minute, but the huntsman doesn't even flinch."
    ""

translate tranditional_chinese battle_flirt_script_29ded62b:

    # gt "Ok... That's too much, you insolent filth. I'm not going to fall for this."
    gt ""

translate tranditional_chinese battle_flirt_script_8256a0e6:

    # gt "Well..."
    gt ""

translate tranditional_chinese battle_flirt_script_5192f25a:

    # "The bear snatches at your hand, but you quickly retreat in time."
    ""

translate tranditional_chinese battle_flirt_script_25da557a:

    # bearGuard "Shut it. This is not a warrior's way of battle."
    bearGuard ""

translate tranditional_chinese battle_flirt_script_cba7f25f:

    # "You continue your act for about a minute, but the [target.name!t] just stares at you in confusion."
    ""

translate tranditional_chinese battle_flirt_script_a125f093:

    # "Vurro's jaw wide in awe, you notice some unusual movement in his pants."
    ""

translate tranditional_chinese battle_flirt_script_29cf4811:

    # v "W-wuldon's gonna like fighting against you, won't he?"
    v ""

translate tranditional_chinese battle_flirt_script_449bc6af:

    # "Vurro lowers his fists for a moment, his cheeks soon turn bright red."
    ""

translate tranditional_chinese battle_flirt_script_cf306a0f:

    # v "Mhmm... just don't flirt with Uffe when the time comes, but I have to admit, you do get a reaction out of me."
    v ""

translate tranditional_chinese battle_flirt_script_ce63920f:

    # "Within a few seconds you can already see some movement from under the goat's loincloth. He doesn't say anything, except for licking his lips. His lust is increased by [player_flirt]."
    ""

translate tranditional_chinese battle_flirt_script_fb404ac5:

    # gt "...I-if you do that one more time I'm going to grab that huge ass and never let go..."
    gt ""

translate tranditional_chinese battle_flirt_script_e2711b23:

    # "You notice the goat is floundering, trying his best not to get aroused by your seduction, but his flushed face and pitched tent says it all. His lust is increased by [player_flirt]."
    ""

translate tranditional_chinese battle_flirt_script_9cd9f568:

    # gt "You are w-wasting your time. I'm n-not... I'm not... I- uhh... nooo..."
    gt ""

translate tranditional_chinese battle_flirt_script_531218b5:

    # "The goat huntsman is squirming in reaction to your advance. You can already hear his rapid breathing and grunting, holding his spear tightly. His lust is increased by [player_flirt]."
    ""

translate tranditional_chinese battle_flirt_script_ae4f2742:

    # gt "N-noooo. I c-can't control... my mind. Please..."
    gt ""

translate tranditional_chinese battle_flirt_script_3548818f:

    # "You can tell the huntsman is already playing with himself when his hand goes under his loincloth, staring at your ass intently. His lust is increased by [player_flirt]"
    ""

translate tranditional_chinese battle_flirt_script_0d56d52b:

    # gt "Hnnnngh... I n-need to... cum."
    gt ""

translate tranditional_chinese battle_flirt_script_ddcedc6e:

    # "You can see the bear's clenches his fist tightly, something under his loincloth begins to move, even as he closes his eyes, and remain silent."
    ""

translate tranditional_chinese battle_flirt_script_877a56ed:

    # "His lust is increased by [player_flirt]."
    ""

translate tranditional_chinese battle_flirt_script_bac300f5:

    # bearGuard "S-shut..."
    bearGuard ""

translate tranditional_chinese battle_flirt_script_64fa8444:

    # "The bear pants heavily, his cheeks are alraedy so red, you can see his drool dripping uncontrollably as he watches."
    ""

translate tranditional_chinese battle_flirt_script_877a56ed_1:

    # "His lust is increased by [player_flirt]."
    ""

translate tranditional_chinese battle_flirt_script_c632bd62:

    # "Even if it's just a little, you notice a mild vibration in the bridgeroot's moss, it seems to react easily. His lust is increased by [player_flirt]."
    ""

translate tranditional_chinese battle_flirt_script_edd422d7:

    # "Almost surprisingly, the slime budges, even just for a little."
    ""

translate tranditional_chinese battle_flirt_script_37523263:

    # "Within a few seconds you can already see some movements under the [target.name!t]'s ripped pants."
    ""

translate tranditional_chinese battle_flirt_script_bb33b631:

    # "The sturdy beast man licks his lips, grumbling at your beautiful sight. His lust is increased by [player_flirt]."
    ""

translate tranditional_chinese battle_flirt_script_60331d20:

    # "You notice the [target.name!t] is staring at your crotch, you slightly wink at him and he already looks like he can't breath under such arousal."
    ""

translate tranditional_chinese battle_flirt_script_877a56ed_2:

    # "His lust is increased by [player_flirt]."
    ""

translate tranditional_chinese battle_flirt_script_8f8f53e6:

    # ww "Hmm... Come closer... little prey."
    ww ""

translate tranditional_chinese battle_flirt_script_b075c3a4:

    # ww "You have a nice body, little prey. Our pack would be delighted to see you."
    ww ""

translate tranditional_chinese battle_flirt_script_2534d3ec:

    # gnl "What's underneath? Gnoll wonders..."
    gnl ""

translate tranditional_chinese battle_flirt_script_d8ea3f64:

    # gnl "Come... we play instead."
    gnl ""

translate tranditional_chinese battle_flirt_script_699d0eb4:

    # bd "Well... our brothers would like to greet you."
    bd ""

translate tranditional_chinese battle_flirt_script_92b78b8c:

    # bd "Adventurers like you are so slutty, I'd plow your ass right now."
    bd ""

translate tranditional_chinese battle_flirt_script_bf758ce5:

    # "The [target.name!t] is squirming in reaction to your advance."
    ""

translate tranditional_chinese battle_flirt_script_f1da305a:

    # "You can already hear his rapid breathing and grunting, grasping at his own claws. His lust is increased by [player_flirt]."
    ""

translate tranditional_chinese battle_flirt_script_21e8d611:

    # "You can tell the [target.name!t] is already playing with himself when his claws goes under his pants, staring at your ass intently."
    ""

translate tranditional_chinese battle_flirt_script_877a56ed_3:

    # "His lust is increased by [player_flirt]."
    ""

translate tranditional_chinese battle_flirt_script_aeda22d8:

    # ww "I-I can't hold... back- if you keep being like that."
    ww ""

translate tranditional_chinese battle_flirt_script_ec94cb21:

    # ww "L-little prey, y-our hole is mine. Now give up already and let me... f-fuck."
    ww ""

translate tranditional_chinese battle_flirt_script_a3289262:

    # gnl "Mmmmmph... I sense a wild growth. Can't hold-"
    gnl ""

translate tranditional_chinese battle_flirt_script_f7049853:

    # gnl "No think- Not... g-giving in..."
    gnl ""

translate tranditional_chinese battle_flirt_script_13188d4d:

    # bd "Mmmmph..."
    bd ""

translate tranditional_chinese battle_flirt_script_756769c4:

    # bd "N-not even... c-close."
    bd ""

translate tranditional_chinese battle_escape_surrender_script_cfcbc5f4:

    # "You walk away from the dummy; it appears to patch itself up before going motionless, letting dust settle on it again."
    ""

translate tranditional_chinese battle_escape_surrender_script_0fe6ac61:

    # "You're not sure if you can leave the training with Vurro right now."
    ""

translate tranditional_chinese battle_escape_surrender_script_f321de17:

    # "You try to escape, but the spritebinder's arm yanks you straight back into the fight."
    ""

translate tranditional_chinese battle_escape_surrender_script_89f13d8f:

    # "You can't escape from the arena."
    ""

translate tranditional_chinese battle_escape_surrender_script_d502f36e:

    # "You cannot run away from battle practice..."
    ""

translate tranditional_chinese battle_escape_surrender_script_9bc3bcf2:

    # "You slowly back away from the [target.name!t]'s attack. Attempting to dodge its gooey appendages, you trip and fall on the grass!"
    ""

translate tranditional_chinese battle_escape_surrender_script_94977c72:

    # "Your escape attempt seems to have failed."
    ""

translate tranditional_chinese battle_escape_surrender_script_c0ea3135:

    # "You slowly back down from the goat's attack, turn around, and run as fast as you can. Suddenly, you slip and fall on the trap he set up ealier. Your escape seems to have failed!"
    ""

translate tranditional_chinese battle_escape_surrender_script_4cd25dd1:

    # gt "Haha... Come back here you insolent wretch, I am not done with you yet."
    gt ""

translate tranditional_chinese battle_escape_surrender_script_d4c5f64c:

    # gt "Mhmm... need to improve your running skills as well."
    gt ""

translate tranditional_chinese battle_escape_surrender_script_ca519a13:

    # "You slowly back away from the [target.name!t]'s attack. Attempting to dodge its ghostly form, you trip and fall on the grass!"
    ""

translate tranditional_chinese battle_escape_surrender_script_94977c72_1:

    # "Your escape attempt seems to have failed."
    ""

translate tranditional_chinese battle_escape_surrender_script_13fbd82f:

    # "You slowly back down from the [target.name!t]'s attack, you turn around and run as fast as you can."
    ""

translate tranditional_chinese battle_escape_surrender_script_b1144d70:

    # "But the [target.name!t]'s claw instantly grips onto your tail and you fall on the ground. Your escape seems to have failed!"
    ""

translate tranditional_chinese battle_escape_surrender_script_dd94ae58:

    # ww "Do not ever think about escaping, little prey. You are mine now."
    ww ""

translate tranditional_chinese battle_escape_surrender_script_ed3263e8:

    # "You slowly back away from the [target.name!t]'s attack. Dodging its gooey appendages. You successfully escape from the [target.name!t]!"
    ""

translate tranditional_chinese battle_escape_surrender_script_d5ffdb9d:

    # "You slowly back away from the goat's attack, turn around, and run as fast as you can. The goat throws his spear at you, barely missing your head. You successfully escaped from the huntsman!"
    ""

translate tranditional_chinese battle_escape_surrender_script_1ebf5074:

    # "You slowly back away from the [target.name!t]'s attack. Dodging its ghostly form. You successfully escape from the [target.name!t]!"
    ""

translate tranditional_chinese battle_escape_surrender_script_13fbd82f_1:

    # "You slowly back down from the [target.name!t]'s attack, you turn around and run as fast as you can."
    ""

translate tranditional_chinese battle_escape_surrender_script_50b7b402:

    # "The [target.name!t] tries to outrun you but he trips and falls on the ground, You successfully escaped from the [target.name!t]!"
    ""

translate tranditional_chinese battle_escape_surrender_script_4988e7e1:

    # "You try to surrender to the dummy, pretending to slip and fall on the floor... its stationary stance seem to advise you to leave instead."
    ""

translate tranditional_chinese battle_escape_surrender_script_9d627f20:

    # "You raises your hand, Vurro quickly notices and lowers his arms."
    ""

translate tranditional_chinese battle_escape_surrender_script_710636c4:

    # v "Not going any further? We can stop here."
    v ""

translate tranditional_chinese battle_escape_surrender_script_47f4bdd0:

    # "You fall to your knees, having exhausted all your energy. You feel the slime has overpowered you in every way possible."
    ""

translate tranditional_chinese battle_escape_surrender_script_664ea1cc:

    # "With no possible outcome in sight, you surrender yourself to its gooey grasp..."
    ""

translate tranditional_chinese battle_escape_surrender_script_75066d56:

    # "You fall to your knees, exhausted all your energy, you grasp for breath as you lie on the ground."
    ""

translate tranditional_chinese battle_escape_surrender_script_ec66902a:

    # "Maybe surrendering yourself to the [target.name!t] is the best choice."
    ""

translate tranditional_chinese battle_escape_surrender_script_c69bc257:

    # a "Are we giving up?"
    a ""

translate tranditional_chinese battle_escape_surrender_script_6ef9c502:

    # "The beast man jeers at your submission, and he paces around you, poking you to see if you'd still react."
    ""

translate tranditional_chinese battle_escape_surrender_script_b2a8349a:

    # "You slowly close your eyes and wait for him to decide your fate."
    ""

translate tranditional_chinese battle_escape_surrender_script_c764976e:

    # gt "Ha... At least you are smart enough to give yourself up to our chief. But, since we're here. I have a better plan for you."
    gt ""

translate tranditional_chinese battle_escape_surrender_script_5fa880da:

    # "He laughs at your submission and looks you up and down. Thinking carefully about his next step..."
    ""

translate tranditional_chinese battle_escape_surrender_script_65ca3b04:

    # gt "Well... it seems we're done...heh. I thought our general said you're strong... not strong enough."
    gt ""

translate tranditional_chinese battle_escape_surrender_script_02490d6b:

    # "He looks at you up and down..."
    ""

translate tranditional_chinese battle_ally_script_2fe31d4b:

    # "Amble strikes the core of [enemy.name], causing [target.name] to fall over. He dealt [ally_damage] and stuns [enemy.name] for [stunned.max_rounds] round."
    ""

translate tranditional_chinese battle_ally_script_3ee9df24:

    # "The [enemy.name] is staggered, releasing you from his grasp."
    ""

translate tranditional_chinese battle_ally_script_cee066c4:

    # "It doesn't seem to work on the bridgeroot."
    ""

translate tranditional_chinese battle_ally_script_8970ba2f:

    # "Amble defends the two of you, increasing your defence in this round."
    ""

translate tranditional_chinese level_up_check_1ca53c77:

    # "You have gained [exp_drop] experience points and [found_gold] gold."
    ""

translate tranditional_chinese level_up_check_91f1d522:

    # msg "You are now Level [pc.level]! Check your inventory to allocate your level points."
    msg ""

translate tranditional_chinese lost_gold_check_a0930d76:

    # "You lost [lost_gold] Gold."
    ""

translate tranditional_chinese mimic_battle_89067aa9:

    # "You are facing a mimic chest, it is licking its lips, drooling at your body. You raise your fist in response."
    ""

translate tranditional_chinese mimic_battle_0ef8db8e:

    # "You are facing a mimic chest, it is licking its lips, drooling at your body. You raise your [pc.weapon.name!t] in response."
    ""

translate tranditional_chinese mimic_battle_loop_ae4274dd:

    # "You aim and slash your [pc.weapon.name!t] at the mimic chest, but you simply miss it by inches."
    ""

translate tranditional_chinese mimic_battle_loop_17f6e215:

    # "You aim and shoot your [pc.weapon.name!t] at the mimic, but you simply miss it by inches."
    ""

translate tranditional_chinese mimic_battle_loop_7a3fb095:

    # "You throw your fist at the mimic, but you simply miss it by inches."
    ""

translate tranditional_chinese mimic_battle_loop_c48a128a:

    # "You slash your [pc.weapon.name!t] at the appendages of the mimic, your blade grazes through the chest."
    ""

translate tranditional_chinese mimic_battle_loop_baeaf8d4:

    # "Drops of blood drips through his body."
    ""

translate tranditional_chinese mimic_battle_loop_ff3906a3:

    # "You slam your [pc.weapon.name!t] at the chest, knocking him sideway."
    ""

translate tranditional_chinese mimic_battle_loop_9667df6c:

    # "He growls at you before setting himself down with his tentacles, albeit disheveled."
    ""

translate tranditional_chinese mimic_battle_loop_fc3dc21d:

    # "You run while shooting your [pc.weapon.name!t] at the mimic, knocking him sideway."
    ""

translate tranditional_chinese mimic_battle_loop_9667df6c_1:

    # "He growls at you before setting himself down with his tentacles, albeit disheveled."
    ""

translate tranditional_chinese mimic_battle_loop_b4e63e0a:

    # "You punch into the mimic's stomach, grabbing him and slam him on the ground hard."
    ""

translate tranditional_chinese mimic_battle_loop_24bcc736:

    # "His health decreases by [oa[4]] HP."
    ""

translate tranditional_chinese mimic_battle_loop_8b6fd26f:

    # "You've critically hit the mimic chest, dealing [oa[4]] HP!"
    ""

translate tranditional_chinese mimic_battle_loop_cd0c87b2:

    # "You struggle against the mimic, trying to break free. You dealt [oa[4]] damage to the mimic in the process, his grip has loosen as well."
    ""

translate tranditional_chinese mimic_battle_loop_55a9d34b:

    # "As much as you try to grind your hips against the chest interior, he doesn't flinch, or get aroused."
    ""

translate tranditional_chinese mimic_battle_loop_cac49cb9:

    # "It seems that he is already very aroused."
    ""

translate tranditional_chinese mimic_battle_loop_31430c67:

    # "You back off before he tries to grab a hold of your body."
    ""

translate tranditional_chinese mimic_battle_loop_018f2681:

    # "You slowly back down from the mimic's attack, you turn around and run as fast as you can."
    ""

translate tranditional_chinese mimic_battle_loop_68646913:

    # "The mimic catches you with his appendages and flings your body right back to him. Your escape seems to have failed!"
    ""

translate tranditional_chinese mimic_battle_loop_018f2681_1:

    # "You slowly back down from the mimic's attack, you turn around and run as fast as you can."
    ""

translate tranditional_chinese mimic_battle_loop_15b24e0c:

    # "The mimic tries to catch you with his appendages but it barely slips from your body, You successfully escaped from the mimic!"
    ""

translate tranditional_chinese mimic_battle_loop_abbbc7d8:

    # "You fall to your knees, exhausted all your energy, you grasp for breath as you lie on the ground, surrendering yourself to the mimic."
    ""

translate tranditional_chinese mimic_battle_loop_4316f2c0:

    # "The mimic licks you full of slime while it seems to be prepares its loser for something bigger..."
    ""

translate tranditional_chinese mimic_battle_loop_5ac9b1f2:

    # k "Courier? W-what are you doing..."
    k ""

translate tranditional_chinese mimic_battle_loop_8aab012c:

    # "The mimic flings his tongue towards you, you are not quick enough to dodge his blow. Your health decreases by [enemy_damage] HP."
    ""

translate tranditional_chinese mimic_battle_loop_8aa01276:

    # "The mimic slaps his tongue onto your head, knocking you on the ground. Your health decreases by [enemy_damage] HP."
    ""

translate tranditional_chinese mimic_battle_loop_32ebdbf5:

    # "The mimic flings his tongue towards you, but you manage to dodge the attack."
    ""

translate tranditional_chinese mimic_battle_loop_de9dffaf:

    # "The mimic tries to slap his tongue onto your head, but he misses it by inches."
    ""

translate tranditional_chinese mimic_battle_loop_a57eb308:

    # "The mimic wraps his tongues and appendages around you."
    ""

translate tranditional_chinese mimic_battle_loop_ff019558:

    # "He is holding you in place."
    ""

translate tranditional_chinese mimic_battle_loop_2abeec4f:

    # "The mimic raises his tongue, smearing all his pheromones onto your skin."
    ""

translate tranditional_chinese mimic_battle_loop_981402c5:

    # "You instantly gets aroused by his slime. Thinking about how his tongue would fit inside you... Your lust increased by [enemy_flirt]."
    ""

translate tranditional_chinese stoneward_battle_8e8755b5:

    # "You are facing a stone ward, it seems to be summoned by the guardian. You raise your fist in defence."
    ""

translate tranditional_chinese stoneward_battle_b6e183cc:

    # "You are facing a stone ward, it seems to be summoned by the guardian. You raise your [pc.weapon.name!t] in defence."
    ""

translate tranditional_chinese stoneward_battle_loop_536191df:

    # "You slash your [pc.weapon.name!t] at the arm of the stone ward, but you simply missed him by inches."
    ""

translate tranditional_chinese stoneward_battle_loop_301ee13f:

    # "You slam your [pc.weapon.name!t] at the stone ward's head, but you simply missed him by inches."
    ""

translate tranditional_chinese stoneward_battle_loop_c9c49552:

    # "You aim and shoot your [pc.weapon.name!t] at the stone ward, but you simply missed him by inches."
    ""

translate tranditional_chinese stoneward_battle_loop_ad49353b:

    # "You throw your fist at the stone ward, but you simply missed him by inches."
    ""

translate tranditional_chinese stoneward_battle_loop_ad1fbfe3:

    # "You slash your [pc.weapon.name!t] at the arm of the stone ward, knocking a few pebbles off his body."
    ""

translate tranditional_chinese stoneward_battle_loop_3f10a603:

    # "He growls at you before getting up, albeit disheveled."
    ""

translate tranditional_chinese stoneward_battle_loop_619d496e:

    # "You slam your [pc.weapon.name!t] at the stone ward's head, knocking him on the ground."
    ""

translate tranditional_chinese stoneward_battle_loop_3f10a603_1:

    # "He growls at you before getting up, albeit disheveled."
    ""

translate tranditional_chinese stoneward_battle_loop_b6ec3400:

    # "You aim and shoot your [pc.weapon.name!t] at the stone ward, the arrow hit right into his shoulder."
    ""

translate tranditional_chinese stoneward_battle_loop_59661794:

    # "You throw your fist at the stone ward, hitting him right across his face."
    ""

translate tranditional_chinese stoneward_battle_loop_d3633215:

    # "The sheer impact knocks a few rocks on the ground."
    ""

translate tranditional_chinese stoneward_battle_loop_24bcc736:

    # "His health decreases by [oa[4]] HP."
    ""

translate tranditional_chinese stoneward_battle_loop_232d37cb:

    # "You've critically hit the stone ward, dealing [oa[4]] HP!"
    ""

translate tranditional_chinese stoneward_battle_loop_43633e63:

    # "As much as you move your hip and grind your ass against the stone ward, it doesn't seem to flinch."
    ""

translate tranditional_chinese stoneward_battle_loop_b8342486:

    # "You eventually give up before he can actually latch onto you."
    ""

translate tranditional_chinese stoneward_battle_loop_60e05e86:

    # "You slowly back down from the stoneward's attack, you turn around and run as fast as you can."
    ""

translate tranditional_chinese stoneward_battle_loop_07717b12:

    # "Suddenly you slip and fall on the ground! Your escape has failed."
    ""

translate tranditional_chinese stoneward_battle_loop_f9e4d13c:

    # "You slowly back down from the stone ward's attack, you turn around and run as fast as you can."
    ""

translate tranditional_chinese stoneward_battle_loop_08db627e:

    # "The stone ward tries to run after you but he is too slow, You successfully escaped from the Stone Ward!"
    ""

translate tranditional_chinese stoneward_battle_loop_c2773c3b:

    # "You fall to your knees, exhausted all your energy, you grasp for breath as you lie on the ground, surrendering yourself to the Stone Ward."
    ""

translate tranditional_chinese stoneward_battle_loop_5ac9b1f2:

    # k "Courier? W-what are you doing..."
    k ""

translate tranditional_chinese stoneward_battle_loop_57148fc2:

    # "The stone ward aims and flings 3 huge stones at you, it ignores your dodges and hit you right onto your body. Your health decreases by [ed] HP."
    ""

translate tranditional_chinese stoneward_battle_loop_73bc9dc4:

    # "The stone ward swings his left arm towards you, you are not quick enough to dodge his blow. Your health decreases by [enemy_damage] HP."
    ""

translate tranditional_chinese stoneward_battle_loop_e7dd5942:

    # "The stone ward charges at you, hitting you with a punch to the chest. Your health decreases by [enemy_damage] HP."
    ""

translate tranditional_chinese stoneward_battle_loop_ff3ccb52:

    # "The stone ward swings his left arm towards you, but you managed to deflect his attack."
    ""

translate tranditional_chinese stoneward_battle_loop_cd981659:

    # "The stone ward charges at you, trying to punch at your chest but you block the blow and push him back."
    ""

translate tranditional_chinese stoneward_battle_loop_baadb9ff:

    # "The stone ward holds his fist, saving himself some pebbles."
    ""

translate tranditional_chinese stoneward_battle_loop_ae5d07b3:

    # "You can feel that he is preparing an attack next round."
    ""

translate tranditional_chinese stoneward_battle_loop_72b11df8:

    # "The stone ward uses the power of the flowing water, and recovers [healing] HP."
    ""

translate tranditional_chinese stoneward_win_337e9c24:

    # "As you defeat the stone ward, the ward begins to collapse and turns to dust in front of your eyes."
    ""

translate tranditional_chinese stoneward_win_e92d4827:

    # "There's nothing useful for now..."
    ""

translate tranditional_chinese stoneward_win_86e38b05:

    # "Searching around the stone ward, you found a normal stone and a slate rock!"
    ""

translate tranditional_chinese stoneward_win_bc0f6ea8:

    # "Searching around the stone ward, you found a normal stone!"
    ""

translate tranditional_chinese stoneward_win_015d740f:

    # "You search around the stone ward, you found a slate rock..."
    ""

translate tranditional_chinese stoneward_win_9a89312e:

    # "You also found [gold_drop] gold and [exp_drop] EXP."
    ""

translate tranditional_chinese stoneward_win_91f1d522:

    # msg "You are now Level [pc.level]! Check your inventory to allocate your level points."
    msg ""

translate tranditional_chinese mimic_win_906e2f2a:

    # "As you defeat the mimic, the entity seems to disintegrate completely before your eyes."
    ""

translate tranditional_chinese mimic_win_3a39d780:

    # "Only leaving behind the content of its original chest."
    ""

translate tranditional_chinese mimic_win_d58adbfc:

    # "You searches around the mimic, you found a chest key."
    ""

translate tranditional_chinese mimic_win_b34e8987:

    # "You found [gold_drop] gold and [exp_drop] EXP."
    ""

translate tranditional_chinese mimic_win_91f1d522:

    # msg "You are now Level [pc.level]! Check your inventory to allocate your level points."
    msg ""

translate tranditional_chinese stoneward_lose_c997ea5b:

    # "You fell on the ground, the stone ward walks towards you."
    ""

translate tranditional_chinese stoneward_lose_041df875:

    # "You faint soon after seeing the ward."
    ""

translate tranditional_chinese stoneward_lose_a20cefa7:

    # "..."
    ""

translate tranditional_chinese stoneward_lose_f950d696:

    # k "You... woke up?"
    k ""

translate tranditional_chinese stoneward_lose_efc11d33:

    # e "Ahh... where am I..."
    e ""

translate tranditional_chinese stoneward_lose_1821d57b:

    # k "Outside of the cave. I healed you for a bit."
    k ""

translate tranditional_chinese stoneward_lose_4784bc78:

    # e "Thanks, Kari."
    e ""

translate tranditional_chinese stoneward_lose_5f047671:

    # k "You lost to a rock."
    k ""

translate tranditional_chinese stoneward_lose_9abcd80b:

    # e "Uhmm... I don't know..."
    e ""

translate tranditional_chinese stoneward_lose_bb92e2b8:

    # k "Whatever, I'm waiting here."
    k ""

translate tranditional_chinese stoneward_lose_e16b34a6:

    # k "Just come back to the cave when you're ready."
    k ""

translate tranditional_chinese stoneward_lose_7020272d:

    # k "I still have to save Furkan..."
    k ""

translate tranditional_chinese stoneward_lose_bf6f0d3d:

    # e "A-alright..."
    e ""

translate tranditional_chinese stoneward_lose_c997ea5b_1:

    # "You fell on the ground, the stone ward walks towards you."
    ""

translate tranditional_chinese stoneward_lose_041df875_1:

    # "You faint soon after seeing the ward."
    ""

translate tranditional_chinese stoneward_lose_38e4b854:

    # "When you wake up, you discovered that you are already outside of the cave..."
    ""

translate tranditional_chinese stoneward_lose_6efd68b1:

    # "You wonder what happened when you passed out..."
    ""

translate tranditional_chinese stoneward_lose_f8ac486d:

    # "But you decide to pick up your items and continue on your adventure."
    ""

translate tranditional_chinese stoneward_lose_4ba96869:

    # "You lost [gold_lost] gold."
    ""

translate tranditional_chinese stoneward_lose_4ba96869_1:

    # "You lost [gold_lost] gold."
    ""

translate tranditional_chinese mimic_lose_5784125a:

    # "You fell on the ground, the mimic's tongue is about to slither towards you."
    ""

translate tranditional_chinese mimic_lose_3be38474:

    # "There's no strength left inside you to struggle against its grasp, it is bringing you into its own mouth..."
    ""

translate tranditional_chinese mimic_lose_392ce5ec:

    # "Just as the mimic is about to engulf you whole, Kari strikes his scepter against the chest and it releases its grip."
    ""

translate tranditional_chinese mimic_lose_1e94c0db:

    # "You fall on the ground hard. Kari carries your half-limp body out of the dungeon and into somewhere safe."
    ""

translate tranditional_chinese mimic_lose_f950d696:

    # k "You... woke up?"
    k ""

translate tranditional_chinese mimic_lose_efc11d33:

    # e "Ahh... where am I..."
    e ""

translate tranditional_chinese mimic_lose_1821d57b:

    # k "Outside of the cave. I healed you for a bit."
    k ""

translate tranditional_chinese mimic_lose_4784bc78:

    # e "Thanks, Kari."
    e ""

translate tranditional_chinese mimic_lose_8f571eff:

    # k "You lost to a chest."
    k ""

translate tranditional_chinese mimic_lose_9abcd80b:

    # e "Uhmm... I don't know..."
    e ""

translate tranditional_chinese mimic_lose_bb92e2b8:

    # k "Whatever, I'm waiting here."
    k ""

translate tranditional_chinese mimic_lose_e16b34a6:

    # k "Just come back to the cave when you're ready."
    k ""

translate tranditional_chinese mimic_lose_7020272d:

    # k "I still have to save Furkan..."
    k ""

translate tranditional_chinese mimic_lose_bf6f0d3d:

    # e "A-alright..."
    e ""

translate tranditional_chinese mimic_lose_a6dbbcdf:

    # "You stand up... feeling something inside you had changed."
    ""

translate tranditional_chinese mimic_lose_88c58a71:

    # "You can't point out what happened or what changed, but your body feels... weird after losing to the mimic."
    ""

translate tranditional_chinese mimic_lose_fde7f20e:

    # "Like... it's not your own."
    ""

translate tranditional_chinese mimic_lose_4ba96869:

    # "You lost [gold_lost] gold."
    ""

translate tranditional_chinese runeguardian_battle_528eb585:

    # "It is a Rune Guardian. He is standing before you, he looks enraged by your intrusion."
    ""

translate tranditional_chinese runeguardian_battle_8ed9f221:

    # "You can feel his blue aura radiates across the cave."
    ""

translate tranditional_chinese runeguardian_battle_456c4288:

    # "You raise your [pc.weapon.name!t], defending yourself from the guardian's attack."
    ""

translate tranditional_chinese runeguardian_battle_35c2da0e:

    # "You raise your fist, defending yourself from the guardian's attack."
    ""

translate tranditional_chinese runeguardian_battle_loop_d7e5bd08:

    # "You slash your [pc.weapon.name!t] at the arm of the guardian, but his magical aura repels the attack."
    ""

translate tranditional_chinese runeguardian_battle_loop_7fecda09:

    # "You slam your [pc.weapon.name!t] at the guardian's head, but his magical aura repels the attack."
    ""

translate tranditional_chinese runeguardian_battle_loop_3aed010f:

    # "You aim and shoot your [pc.weapon.name!t] at the guardian, but it seems to have missed."
    ""

translate tranditional_chinese runeguardian_battle_loop_da1015f3:

    # "You throw your fist at the guardian, but you punch into the air instead."
    ""

translate tranditional_chinese runeguardian_battle_loop_34af9287:

    # "You slash your [pc.weapon.name!t] at the arm of the Rune Guardian, your blade scraps against the stone. Some of it crack and falls off his body."
    ""

translate tranditional_chinese runeguardian_battle_loop_b1e6fa40:

    # "You slash your [pc.weapon.name!t] across the guardian's body, knocking him back a few steps. The guardian quakes silently in anger."
    ""

translate tranditional_chinese runeguardian_battle_loop_0e24220d:

    # "You slam your [pc.weapon.name!t] at the arm of the Rune Guardian, your blade grazes against the stone. Some of it crack and falls off his body."
    ""

translate tranditional_chinese runeguardian_battle_loop_124a16cb:

    # "You slam your [pc.weapon.name!t] across the guardian's body, knocking him back a few steps. The guardian quakes silently in anger."
    ""

translate tranditional_chinese runeguardian_battle_loop_7c6ad50c:

    # "You aim and shoot your [pc.weapon.name!t] at the Rune Guardian, the arrow hits right into his arm, striking off a few rubbles."
    ""

translate tranditional_chinese runeguardian_battle_loop_db8f38b0:

    # "You run while shooting your [pc.weapon.name!t] across the guardian's body, knocking him back a few steps. The guardian quakes silently in anger."
    ""

translate tranditional_chinese runeguardian_battle_loop_7ebd82f2:

    # "You throw your fist at the guardian, hitting him right across his body, the sheer impact strikes off a few rubbles."
    ""

translate tranditional_chinese runeguardian_battle_loop_46df7f65:

    # "You punch into the guardian's stomach, knocking him back a few steps. The guardian quakes silently in anger."
    ""

translate tranditional_chinese runeguardian_battle_loop_24bcc736:

    # "His health decreases by [oa[4]] HP."
    ""

translate tranditional_chinese runeguardian_battle_loop_26f13d1d:

    # "You've critically hit the guardian, dealing [oa[4]] HP!"
    ""

translate tranditional_chinese runeguardian_battle_loop_9f89fa4c:

    # "You struggle against the spell, trying to break free. You dealt [oa[4]] damage to the guardian in the process, his grip has loosen as well."
    ""

translate tranditional_chinese runeguardian_battle_loop_92d18f4a:

    # "You struggle against the guardian as you try to reach under the guardian's crotch, trying to get a reaction from the guardian."
    ""

translate tranditional_chinese runeguardian_battle_loop_58222c25:

    # "The guardian instanly react with your advance, vibrating profusely with his moss. His grip is weakening as well."
    ""

translate tranditional_chinese runeguardian_battle_loop_fdf9de27:

    # "You turn around and rub your hand all over your own burly cheeks, feeling and brushing against your ass while you shake your hip."
    ""

translate tranditional_chinese runeguardian_battle_loop_58ea5730:

    # "You scrape your member lightly, running your claw from your inner thigh to the back of your balls, you tug at it tightly while staring at the Rune Guardian seductively."
    ""

translate tranditional_chinese runeguardian_battle_loop_651e30ea:

    # "You cup at your fluffy chest, drawing circles around the area of your nipples. You smile at the Rune Guardian while your chest bounce up and down slightly."
    ""

translate tranditional_chinese runeguardian_battle_loop_24a12d2a:

    # "You approach to the guardian closely, letting your hands wander inside the huge bush in his crotch area."
    ""

translate tranditional_chinese runeguardian_battle_loop_e8fc8231:

    # "You continue your act for about a minute, but the Rune Guardian doesn't even flinch."
    ""

translate tranditional_chinese runeguardian_battle_loop_4c1908a3:

    # "Disappointed, you back away before the guardian can grab a hold of you."
    ""

translate tranditional_chinese runeguardian_battle_loop_c294968b:

    # "You cannot detect his lust on the face, but by the vibration of the moss you can deduce that he's enjoying this a lot. His lust is increased by [player_flirt]."
    ""

translate tranditional_chinese runeguardian_battle_loop_0686837e:

    # "The guardian doesn't speak, but he is extremely distracted by your performance. His lust is increased by [player_flirt]."
    ""

translate tranditional_chinese runeguardian_battle_loop_449a6dcd:

    # "As much as you try, you cannot escape from the guardian's magical aura."
    ""

translate tranditional_chinese runeguardian_battle_loop_f800fe89:

    # "You fall to your knees, exhausted all your energy, you grasp for breath as you lie on the ground, surrendering yourself to the guardian."
    ""

translate tranditional_chinese runeguardian_battle_loop_41dccd4c:

    # "The guardian falls with a huge thud on the ground."
    ""

translate tranditional_chinese runeguardian_battle_loop_a2b396b9:

    # "You don't know if your flirt worked, but it certainly works by putting down the guardian."
    ""

translate tranditional_chinese runeguardian_battle_loop_57148fc2:

    # "The stone ward aims and flings 3 huge stones at you, it ignores your dodges and hit you right onto your body. Your health decreases by [ed] HP."
    ""

translate tranditional_chinese runeguardian_battle_loop_8133aa7b:

    # "The Rune Guardian holds you in place with his right arm. You try to struggle free but it doesn't work."
    ""

translate tranditional_chinese runeguardian_battle_loop_8dd06c89:

    # "The guardian swing his fist toward your direction, hitting you in the chest. Your health decreases by [enemy_damage] HP."
    ""

translate tranditional_chinese runeguardian_battle_loop_f6c92ffb:

    # "The guardian strike you down with his vines, you pass out for a few seconds before getting up. Your health decreases by [enemy_damage] HP."
    ""

translate tranditional_chinese runeguardian_battle_loop_9399e2c4:

    # "The guardian swing his fist toward your direction, but you manage to dodge his blow."
    ""

translate tranditional_chinese runeguardian_battle_loop_d4f76538:

    # "The guardian tries to strike you down with his vines, but he missed the attack just by inches."
    ""

translate tranditional_chinese runeguardian_battle_loop_edbb71f1:

    # "The guardian prepares for his magic... you feel like his next attack would hurt a lot more."
    ""

translate tranditional_chinese runeguardian_battle_loop_59d3dccc:

    # k "Courier, watch out for his attack!"
    k ""

translate tranditional_chinese runeguardian_battle_loop_af541553:

    # "The Rune Guardian channels the water into an orb of regeneration, his body becomes revitalised by the power of water."
    ""

translate tranditional_chinese runeguardian_battle_loop_8fd8c645:

    # "He has recovered [healing] HP."
    ""

translate tranditional_chinese runeguardian_win_d87424a9:

    # "The Rune Guardian collapse on the ground, you stare at the guardian, its blue marks are glowly weakly."
    ""

translate tranditional_chinese runeguardian_win_a5b8bed6:

    # "Holding the stone against the golem. The shell of the rock crumbles and reveals a rune crystal right before your eyes."
    ""

translate tranditional_chinese runeguardian_win_ddbc5ca2:

    # k "W-what?"
    k ""

translate tranditional_chinese runeguardian_win_81daaa47:

    # e "I think it's glowing..."
    e ""

translate tranditional_chinese runeguardian_win_82091822:

    # k "..."
    k ""

translate tranditional_chinese runeguardian_win_7469aa9c:

    # "The rune fuses into the chest of the dying guardian, and the whole cave glows with bright blue energy."
    ""

translate tranditional_chinese runeguardian_win_7df9c133:

    # "Soon, everything returns to normal, except that the guardian seems to not disintegrate like the last one did."
    ""

translate tranditional_chinese runeguardian_win_da5ef959:

    # k "It was a weird stone... how did you know it'd save the guardian?"
    k ""

translate tranditional_chinese runeguardian_win_69a09c13:

    # e "I don't know... I just thought it looked cool."
    e ""

translate tranditional_chinese runeguardian_win_39d2cf8c:

    # k "I think the stone contains some... remnant of the rune..."
    k ""

translate tranditional_chinese runeguardian_win_82091822_1:

    # k "..."
    k ""

translate tranditional_chinese runeguardian_win_dca44186:

    # k "At least we can salvage Naim now."
    k ""

translate tranditional_chinese runeguardian_win_e5b03714:

    # e "Is it this guardian's name?"
    e ""

translate tranditional_chinese runeguardian_win_64ac4548:

    # k "Yes."
    k ""

translate tranditional_chinese runeguardian_win_72a33094:

    # e "..."
    e ""

translate tranditional_chinese runeguardian_win_719a1cd2:

    # k "Let him rest here a little."
    k ""

translate tranditional_chinese runeguardian_win_51770df3:

    # "The guardian stays unconscious after the fight, you decide to check in on Furkan at the back."
    ""

translate tranditional_chinese runeguardian_win_5121ca19:

    # "Soon, the guardian crumbles into dust, just like the last one."
    ""

translate tranditional_chinese runeguardian_win_6fb165f8:

    # k "Hmm..."
    k ""

translate tranditional_chinese runeguardian_win_8d125bb3:

    # k "Now both of them are gone now..."
    k ""

translate tranditional_chinese runeguardian_win_6c4c18c3:

    # "Stepping over the dust that the guardian leaves behind, you decide to check in on Furkan at the back."
    ""

translate tranditional_chinese Damp_Cave_End_ef6b3876:

    # k "Hey, Furk. Furk! You alright?"
    k ""

translate tranditional_chinese Damp_Cave_End_a17ba0e9:

    # "The Ram doesn't speak, in fact he still hasn't wake. Kari presses on his chest for a few moment."
    ""

translate tranditional_chinese Damp_Cave_End_f397e375:

    # k "Alright, he's alive. Furk."
    k ""

translate tranditional_chinese Damp_Cave_End_46661de9:

    # "Kari performs healing magic to the unconscious ram."
    ""

translate tranditional_chinese Damp_Cave_End_9a2b7cf9:

    # "Furkan's eyes flutter open, as an anticipated deer watches in relief."
    ""

translate tranditional_chinese Damp_Cave_End_3ec57d49:

    # f "..."
    f ""

translate tranditional_chinese Damp_Cave_End_9618a388:

    # k "Furk?"
    k ""

translate tranditional_chinese Damp_Cave_End_02545655:

    # f "...W-where am I?"
    f ""

translate tranditional_chinese Damp_Cave_End_00f52772:

    # "Kari hugs the chief tightly, he positions himself on top of Furkan, feeling all his warmth as you watch from a distance."
    ""

translate tranditional_chinese Damp_Cave_End_068112a1:

    # k "Furk, you almost scared me to death. I thought you were gone."
    k ""

translate tranditional_chinese Damp_Cave_End_b7cc3345:

    # f "Uhm... the cave... I came here looking for the golem did I not?"
    f ""

translate tranditional_chinese Damp_Cave_End_723c6101:

    # "Kari nods."
    ""

translate tranditional_chinese Damp_Cave_End_48014797:

    # e "Hey, Furkan."
    e ""

translate tranditional_chinese Damp_Cave_End_7c5afb1c:

    # f "The courier? Why are you here."
    f ""

translate tranditional_chinese Damp_Cave_End_e5aaaaa3:

    # e "Kari asked me to come with him to save you."
    e ""

translate tranditional_chinese Damp_Cave_End_f4847294:

    # e "Did you pass out after the guardian attacked you?"
    e ""

translate tranditional_chinese Damp_Cave_End_ae4b54e1:

    # f "No, it was someone else... the guardian was protecting me."
    f ""

translate tranditional_chinese Damp_Cave_End_3bea46d2:

    # k "Please tell me who attacked you, Furkan. I will take my own revenge on them."
    k ""

translate tranditional_chinese Damp_Cave_End_000b35c8:

    # f "They hit me from the back, I have no idea."
    f ""

translate tranditional_chinese Damp_Cave_End_d4a36c19:

    # "Kari looks disappointed, it seems he cares about the chief a lot by the way he's still hugging him."
    ""

translate tranditional_chinese Damp_Cave_End_4312d7b7:

    # f "W-wait..."
    f ""

translate tranditional_chinese Damp_Cave_End_9f94dcc8:

    # f "I lost it."
    f ""

translate tranditional_chinese Damp_Cave_End_6a9d299d:

    # k "Father's basin?"
    k ""

translate tranditional_chinese Damp_Cave_End_32a6dc7b:

    # "The general pulls away, looking at Furkan with concern."
    ""

translate tranditional_chinese Damp_Cave_End_1b47d087:

    # k "Oh... No."
    k ""

translate tranditional_chinese Damp_Cave_End_ead24320:

    # e "W-what's it?"
    e ""

translate tranditional_chinese Damp_Cave_End_07929565:

    # k "We used it to summon our guardians, or what you called golem."
    k ""

translate tranditional_chinese Damp_Cave_End_b34597c1:

    # f "Is the guardian doing well?"
    f ""

translate tranditional_chinese Damp_Cave_End_001113cc:

    # k "Courier used a rock on him, I saw it."
    k ""

translate tranditional_chinese Damp_Cave_End_93902d70:

    # "You nods slightly as the chief gives you an approving smile."
    ""

translate tranditional_chinese Damp_Cave_End_46a16c41:

    # "But the smile fades away quickly as he looks back at Kari."
    ""

translate tranditional_chinese Damp_Cave_End_5363b379:

    # f "It was stupid of me to come here alone. I did not intend for anyone to follow me."
    f ""

translate tranditional_chinese Damp_Cave_End_deb4c568:

    # f "But that made the perfect opportunity for an ambush."
    f ""

translate tranditional_chinese Damp_Cave_End_54f08db2:

    # f "How silly I am..."
    f ""

translate tranditional_chinese Damp_Cave_End_58798f39:

    # k "Furk... it's fine. we can find the basin later."
    k ""

translate tranditional_chinese Damp_Cave_End_68914fef:

    # f "Where's the guardian?"
    f ""

translate tranditional_chinese Damp_Cave_End_98e8204b:

    # k "We killed it. We thought he went rogue on you and he started attacking us."
    k ""

translate tranditional_chinese Damp_Cave_End_f273e2bb:

    # f "Alright."
    f ""

translate tranditional_chinese Damp_Cave_End_fa11c99b:

    # "Furkan sighs, he looks down, eyes almost teary."
    ""

translate tranditional_chinese Damp_Cave_End_a083357a:

    # "He doesn't speak, instead, he grasps on Kari's hand. The general doesn't seem to flich away."
    ""

translate tranditional_chinese Damp_Cave_End_ab4dee2a:

    # "Both of them are just mourning, in their own way."
    ""

translate tranditional_chinese Damp_Cave_End_f2da58de:

    # f "So, all of what I did was for nothing."
    f ""

translate tranditional_chinese Damp_Cave_End_87555c13:

    # f "Guardian died, basin got stolen."
    f ""

translate tranditional_chinese Damp_Cave_End_db3cd737:

    # e "W-what's the basin actually?"
    e ""

translate tranditional_chinese Damp_Cave_End_f2575d7f:

    # f "It was what my father used to summon guardians, with the help of the runes."
    f ""

translate tranditional_chinese Damp_Cave_End_095e395f:

    # f "In return, the guardians protect the runes."
    f ""

translate tranditional_chinese Damp_Cave_End_cd0075c1:

    # f "And now I lost both of them."
    f ""

translate tranditional_chinese Damp_Cave_End_c3ed46ef:

    # "Kari rubs his paws on Furkan's wound, tending to it carefully."
    ""

translate tranditional_chinese Damp_Cave_End_a534077e:

    # e "I'm really sorry to hear, but we'll try to get them back when you recover."
    e ""

translate tranditional_chinese Damp_Cave_End_cebe6de1:

    # f "Thanks, both of you. I need to get back to the tribe now."
    f ""

translate tranditional_chinese Damp_Cave_End_8c3907b5:

    # f "Hmm..."
    f ""

translate tranditional_chinese Damp_Cave_End_486f5a9b:

    # "Furkan stumbles forward when he tries to stand up, you see Kari intuitively lifts him back up."
    ""

translate tranditional_chinese Damp_Cave_End_6cd031f3:

    # f "Kari?"
    f ""

translate tranditional_chinese Damp_Cave_End_6d3d914b:

    # k "Yeah... Let's go."
    k ""

translate tranditional_chinese Damp_Cave_End_e73388e9:

    # e "Oh... you guys... are going?"
    e ""

translate tranditional_chinese Damp_Cave_End_a944e8ba:

    # "Kari carries the furkan's exhausted body on his back. The two men grunts a little before walking."
    ""

translate tranditional_chinese Damp_Cave_End_6409673d:

    # k "U-uhm... Thanks, Courier."
    k ""

translate tranditional_chinese Damp_Cave_End_b213370a:

    # k "Here's some recipe that you might want to take a look."
    k ""

translate tranditional_chinese Damp_Cave_End_44668cc5:

    # k "It's iron sword and axe."
    k ""

translate tranditional_chinese Damp_Cave_End_a3ae056d:

    # e "Thanks, general."
    e ""

translate tranditional_chinese Damp_Cave_End_84fcf348:

    # "He hands you the paper, before tugging the chieftain upwards with his hands on his ass."
    ""

translate tranditional_chinese Damp_Cave_End_127daacc:

    # k "I'll take our Chief back to our tribe, see you later."
    k ""

translate tranditional_chinese Damp_Cave_End_a5a58d4c:

    # f "Take care."
    f ""

translate tranditional_chinese Damp_Cave_End_ce1cdeaf:

    # e "See you..."
    e ""

translate tranditional_chinese Damp_Cave_End_e0df77ea:

    # "Kari turns away from you, and begin walking towards the exit."
    ""

translate tranditional_chinese Damp_Cave_End_4c3c3c00:

    # k "{size=20}Furk, since when did you get this heavy.{/size}"
    k ""

translate tranditional_chinese Damp_Cave_End_2b1a552a:

    # "They whispers, but the echo in the cave makes it clear even from afar."
    ""

translate tranditional_chinese Damp_Cave_End_b055cfae:

    # f "{size=20}Uhmm... Heavier if I fall asleep on your shoulder...{/size}"
    f ""

translate tranditional_chinese Damp_Cave_End_747772b0:

    # "Kari and Furkan leaves you outside the cave."
    ""

translate tranditional_chinese Damp_Cave_End_ed08a457:

    # e "Fuck, Furkan? Are you awake?"
    e ""

translate tranditional_chinese Damp_Cave_End_ad844077:

    # "The Ram is completely passed out, you decide to presses on his chest for a few moment."
    ""

translate tranditional_chinese Damp_Cave_End_81fffab3:

    # "With no avail, you lean your head against his chest, luckily his heart is still beating, albeit weakly."
    ""

translate tranditional_chinese Damp_Cave_End_282a297e:

    # "You cast a weak healing magic on him."
    ""

translate tranditional_chinese Damp_Cave_End_3ec57d49_1:

    # f "..."
    f ""

translate tranditional_chinese Damp_Cave_End_fca3c10c:

    # "The chief's eyes flutter open. He looks stressful, and his pupils dilate."
    ""

translate tranditional_chinese Damp_Cave_End_4e056dcb:

    # f "...W-who..."
    f ""

translate tranditional_chinese Damp_Cave_End_2f3870eb:

    # e "Hey, Furkan. Are you alright?"
    e ""

translate tranditional_chinese Damp_Cave_End_bcda30d6:

    # "He turns his attention to you."
    ""

translate tranditional_chinese Damp_Cave_End_d06b9227:

    # f "Courier? Why are you here."
    f ""

translate tranditional_chinese Damp_Cave_End_25d2b6ed:

    # e "Your general told me to save you here..."
    e ""

translate tranditional_chinese Damp_Cave_End_f0e70fe2:

    # e "He's resting in the tribe now."
    e ""

translate tranditional_chinese Damp_Cave_End_a3e1f029:

    # "You try to give him a hand as he shifts his position to you, but he doesn't take it."
    ""

translate tranditional_chinese Damp_Cave_End_9f93daa8:

    # f "W-what happened."
    f ""

translate tranditional_chinese Damp_Cave_End_713df877:

    # e "He fought me as I entered your tribe, I defeated him at the end."
    e ""

translate tranditional_chinese Damp_Cave_End_c492b7b4:

    # e "I didn't intend to, just trying to protect myself. He thought I kidnapped you."
    e ""

translate tranditional_chinese Damp_Cave_End_b329270f:

    # e "Afterwards he told me you might be here..."
    e ""

translate tranditional_chinese Damp_Cave_End_bbdc0855:

    # "Furkan stares forwards, seemingly pondering something you're not aware of."
    ""

translate tranditional_chinese Damp_Cave_End_a9d62d89:

    # e "Uhm... Furkan... did you pass out after the guardian attacked you?"
    e ""

translate tranditional_chinese Damp_Cave_End_90f09731:

    # f "No... it was someone else... the guardian was protecting me."
    f ""

translate tranditional_chinese Damp_Cave_End_9168b5c7:

    # "He looks around, distressed."
    ""

translate tranditional_chinese Damp_Cave_End_4312d7b7_1:

    # f "W-wait..."
    f ""

translate tranditional_chinese Damp_Cave_End_9f94dcc8_1:

    # f "I lost it."
    f ""

translate tranditional_chinese Damp_Cave_End_32638e3f:

    # f "I lost the basin."
    f ""

translate tranditional_chinese Damp_Cave_End_4341cf81:

    # f "Oh... no no no..."
    f ""

translate tranditional_chinese Damp_Cave_End_c29fa0c3:

    # e "W-what? The basin?"
    e ""

translate tranditional_chinese Damp_Cave_End_1dd4c735:

    # f "Fuck! Why am I so useless."
    f ""

translate tranditional_chinese Damp_Cave_End_a2ff6800:

    # f "They stole the basin from me. The thieves."
    f ""

translate tranditional_chinese Damp_Cave_End_2c5adaba:

    # e "Who took it? I didn't see anyone else here in the cave."
    e ""

translate tranditional_chinese Damp_Cave_End_6fea8302:

    # f "They're probably gone, I did not get to catch a glimpse but I sensed someone was there."
    f ""

translate tranditional_chinese Damp_Cave_End_560a4f40:

    # f "...Right before I was ambushed from behind."
    f ""

translate tranditional_chinese Damp_Cave_End_33656c76:

    # e "It's ok, Furkan. We can get it back later. You should focus on recovering first."
    e ""

translate tranditional_chinese Damp_Cave_End_a13c0780:

    # "You lend your hand again towards Furkan, this time he reluctantly accepts it."
    ""

translate tranditional_chinese Damp_Cave_End_39000206:

    # "With your assistance, Furkan turns against a huge rock to lean on."
    ""

translate tranditional_chinese Damp_Cave_End_b34597c1_1:

    # f "Is the guardian doing well?"
    f ""

translate tranditional_chinese Damp_Cave_End_8d754181:

    # e "Yes, the stone was healing him quite a bit."
    e ""

translate tranditional_chinese Damp_Cave_End_0e8db784:

    # f "What stone?"
    f ""

translate tranditional_chinese Damp_Cave_End_9520fe9c:

    # "You show him the rock you found at the lake, apparently it's been soaked in magical energy enough to heal a golem."
    ""

translate tranditional_chinese Damp_Cave_End_051170e6:

    # f "Thank god."
    f ""

translate tranditional_chinese Damp_Cave_End_2ff7bb12:

    # "Furkan sighs, he is looking at the guardian, panting."
    ""

translate tranditional_chinese Damp_Cave_End_85b84d75:

    # f "It was stupid of me to come here alone. I know the guardian is here."
    f ""

translate tranditional_chinese Damp_Cave_End_d3d886a4:

    # "He looks down, hands over his face."
    ""

translate tranditional_chinese Damp_Cave_End_deb4c568_1:

    # f "But that made the perfect opportunity for an ambush."
    f ""

translate tranditional_chinese Damp_Cave_End_da4677b2:

    # f "How silly am I..."
    f ""

translate tranditional_chinese Damp_Cave_End_6e1fdc41:

    # e "Look, don't worry about it..."
    e ""

translate tranditional_chinese Damp_Cave_End_68914fef_1:

    # f "Where's the guardian?"
    f ""

translate tranditional_chinese Damp_Cave_End_8fbbec14:

    # e "He attacked me... So I killed it."
    e ""

translate tranditional_chinese Damp_Cave_End_f273e2bb_1:

    # f "Alright."
    f ""

translate tranditional_chinese Damp_Cave_End_fa11c99b_1:

    # "Furkan sighs, he looks down, eyes almost teary."
    ""

translate tranditional_chinese Damp_Cave_End_f2da58de_1:

    # f "So, all of what I did was for nothing."
    f ""

translate tranditional_chinese Damp_Cave_End_87555c13_1:

    # f "Guardian died, basin got stolen."
    f ""

translate tranditional_chinese Damp_Cave_End_db3cd737_1:

    # e "W-what's the basin actually?"
    e ""

translate tranditional_chinese Damp_Cave_End_f2575d7f_1:

    # f "It was what my father used to summon guardians, with the help of the runes."
    f ""

translate tranditional_chinese Damp_Cave_End_095e395f_1:

    # f "In return, the guardians protect the runes."
    f ""

translate tranditional_chinese Damp_Cave_End_cd0075c1_1:

    # f "And now I lost both of them."
    f ""

translate tranditional_chinese Damp_Cave_End_534602b9:

    # "He turns away, ears droop low."
    ""

translate tranditional_chinese Damp_Cave_End_a534077e_1:

    # e "I'm really sorry to hear, but we'll try to get them back when you recover."
    e ""

translate tranditional_chinese Damp_Cave_End_0623b8b8:

    # f "Thanks, Courier. I need to get back to the tribe now."
    f ""

translate tranditional_chinese Damp_Cave_End_8c3907b5_1:

    # f "Hmm..."
    f ""

translate tranditional_chinese Damp_Cave_End_5685146e:

    # "Furkan stumbles forward when he tries to stand up. He lands on the floor again with a loud thump."
    ""

translate tranditional_chinese Damp_Cave_End_3ec57d49_2:

    # f "..."
    f ""

translate tranditional_chinese Damp_Cave_End_6af2229e:

    # f "Courier, can you ask Kari to send his people here?"
    f ""

translate tranditional_chinese Damp_Cave_End_6d9927f9:

    # e "Do you need a hand?"
    e ""

translate tranditional_chinese Damp_Cave_End_ec3f1531:

    # f "No need. You've been a great help already. I just need the guards to take me back."
    f ""

translate tranditional_chinese Damp_Cave_End_9d2f2d12:

    # e "Alright."
    e ""

translate tranditional_chinese Damp_Cave_End_bbec41a3:

    # e "What about the guardian?"
    e ""

translate tranditional_chinese Damp_Cave_End_85ee5c06:

    # f "I suppose we will carry it back."
    f ""

translate tranditional_chinese Damp_Cave_End_1d3e7715:

    # f "Not sure if he will still go rogue, so we will put him in captivity."
    f ""

translate tranditional_chinese Damp_Cave_End_41569b97:

    # f "There is just nothing left for it to protect."
    f ""

translate tranditional_chinese Damp_Cave_End_6619e846:

    # "Furkan takes another pause."
    ""

translate tranditional_chinese Damp_Cave_End_049ac92c:

    # f "You can leave me here now... Thank you again, Courier."
    f ""

translate tranditional_chinese Damp_Cave_End_a20cefa7:

    # "..."
    ""

translate tranditional_chinese Damp_Cave_End_3b4dc56d:

    # "You still don't understand a lot of it... The magical runes, the guardians. the basin..."
    ""

translate tranditional_chinese Damp_Cave_End_415e583b:

    # "But maybe there's more to it than you'd imagine."
    ""

translate tranditional_chinese Damp_Cave_End_f56abb08:

    # "Maybe it all links back to why you arrive to this world in the first place."
    ""

translate tranditional_chinese Damp_Cave_End_4dbb790a:

    # "You desperately want it to be this case."
    ""

translate tranditional_chinese Damp_Cave_End_ebfee94b:

    # "If it is a nightmare, maybe you can find a way to wake up."
    ""

translate tranditional_chinese Damp_Cave_End_23e939ae:

    # "Though..."
    ""

translate tranditional_chinese Damp_Cave_End_4322f566:

    # "What's even wrong with indulging yourself in this, nightmare?"
    ""

translate tranditional_chinese Damp_Cave_End_52f8d5eb:

    # "You've never been in charge for the longest of your life."
    ""

translate tranditional_chinese Damp_Cave_End_c4aaf428:

    # "The world you lived in, in the tribe. It was... bland, unfruitful."
    ""

translate tranditional_chinese Damp_Cave_End_5033d6bd:

    # "You don't even remember any other person there, except for Chime."
    ""

translate tranditional_chinese Damp_Cave_End_86e9aeda:

    # "But here you feel like you are doing something. You have the power to change something, for better or worse."
    ""

translate tranditional_chinese Damp_Cave_End_d9da896d:

    # "Here, you feel powerful, you feel as if there's a flame inside you, waiting to rekindle."
    ""

translate tranditional_chinese Damp_Cave_End_742ea64e:

    # "You don't ever want to leave behind the friends you've made, if you ever found an exit."
    ""

translate tranditional_chinese Damp_Cave_End_ab597d55:

    # "Still, Chime is out there, alone, and you can't let that continue for long."
    ""

translate tranditional_chinese Damp_Cave_End_a20cefa7_1:

    # "..."
    ""

translate tranditional_chinese Damp_Cave_End_ecc36c43:

    # "Quickly... you put all unnecessary thought in the back of your head."
    ""

translate tranditional_chinese Damp_Cave_End_d8ce2a99:

    # "And you continue on your journey in this world."
    ""

translate tranditional_chinese runeguardian_lose_85fa647a:

    # "You fall on the ground, the guardian is trying to approach you."
    ""

translate tranditional_chinese runeguardian_lose_46ce7442:

    # "There's no strength left inside you to struggle against his grasp..."
    ""

translate tranditional_chinese runeguardian_lose_e53930cf:

    # "Just as the guardian is about to cast his lethal spell, Kari strikes his scepter against the guardian and pull you off."
    ""

translate tranditional_chinese runeguardian_lose_effe461f:

    # "Kari carries your half-limp body out of the dungeon and into somewhere safe."
    ""

translate tranditional_chinese runeguardian_lose_f950d696:

    # k "You... woke up?"
    k ""

translate tranditional_chinese runeguardian_lose_efc11d33:

    # e "Ahh... where am I..."
    e ""

translate tranditional_chinese runeguardian_lose_1821d57b:

    # k "Outside of the cave. I healed you for a bit."
    k ""

translate tranditional_chinese runeguardian_lose_4784bc78:

    # e "Thanks, Kari."
    e ""

translate tranditional_chinese runeguardian_lose_47f4b7b6:

    # k "Furkan was just there..."
    k ""

translate tranditional_chinese runeguardian_lose_9abcd80b:

    # e "Uhmm... I don't know..."
    e ""

translate tranditional_chinese runeguardian_lose_bb92e2b8:

    # k "Whatever, I'm waiting here."
    k ""

translate tranditional_chinese runeguardian_lose_e16b34a6:

    # k "Just come back to the cave when you're ready."
    k ""

translate tranditional_chinese runeguardian_lose_7020272d:

    # k "I still have to save Furkan..."
    k ""

translate tranditional_chinese runeguardian_lose_bf6f0d3d:

    # e "A-alright..."
    e ""

translate tranditional_chinese runeguardian_lose_bcc26d1b:

    # "The guardian stares at your vulnerable state, but he doesn't stop here. You realise that you are not making out of this alive..."
    ""

translate tranditional_chinese runeguardian_lose_1b54c835:

    # "He casts another spell. Completely binding your body under his control."
    ""

translate tranditional_chinese runeguardian_lose_5d25027a:

    # e "No..."
    e ""

translate tranditional_chinese runeguardian_lose_113a26b3:

    # "You pass out right before you can sense any pain, you don't know if you should consider this lucky or unlucky..."
    ""

translate tranditional_chinese runeguardian_lose_48836530:

    # "You think about whether Furkan would survive, you are right in front of him when you passed out."
    ""

translate tranditional_chinese runeguardian_lose_a1d841d6:

    # "You wonder what he would think when see you like this..."
    ""

translate tranditional_chinese runeguardian_lose_f3066461:

    # "But... it doesn't matter you are gone now."
    ""

translate tranditional_chinese runeguardian_lose_0326b3ba:

    # "This is the end of your journey."
    ""

translate tranditional_chinese runeguardian_lose_e5543b2f:

    # msg "You died."
    msg ""

translate tranditional_chinese runeguardian_lose_4ba96869:

    # "You lost [gold_lost] gold."
    ""

translate tranditional_chinese minostatue_battle_e42b0c88:

    # "You are facing the statue of minotaur, You feel that this battle will be quite different than you are used to. You raise your fist."
    ""

translate tranditional_chinese minostatue_battle_7a41cd1f:

    # "You are facing the statue of minotaur, You feel that this battle will be quite different than you are used to. You raise your fist instead of your [pc.weapon.name!t]."
    ""

translate tranditional_chinese minostatue_battle_loop_415443c8:

    # "You throw your punch at the statue. Regardless of your strength, the statue's health dropped by 1."
    ""

translate tranditional_chinese minostatue_battle_loop_72486f84:

    # "The statue seems to be unfazed by all your attempts at seduction..."
    ""

translate tranditional_chinese minostatue_battle_loop_6bd76fe0:

    # "You cannot escape from the battle."
    ""

translate tranditional_chinese minostatue_battle_loop_e53087dd:

    # "You fall to your knees, exhausted all your energy, you grasp for breath as you lie on the ground, surrendering yourself to the Statue."
    ""

translate tranditional_chinese minostatue_battle_loop_8273253b:

    # "The statue casts a few pebbles onto you, each dealing [enemy_damage] HP."
    ""

translate tranditional_chinese minostatue_battle_loop_6cd74592:

    # "Your health decreased by [ed] in total."
    ""

translate tranditional_chinese minostatue_win_aafcefe4:

    # "As you defeat the statue, it begins to glow in green in front of your eyes."
    ""

translate tranditional_chinese minostatue_win_1f92bb3e:

    # "You feel that the gem flickered for a moment. You must have completed something..."
    ""

translate tranditional_chinese minostatue_win_9a89312e:

    # "You also found [gold_drop] gold and [exp_drop] EXP."
    ""

translate tranditional_chinese minostatue_win_91f1d522:

    # msg "You are now Level [pc.level]! Check your inventory to allocate your level points."
    msg ""

translate tranditional_chinese minostatue_lose_bff43b3c:

    # "You lost to the statue. It seems to be unfazed still."
    ""

translate tranditional_chinese minostatue_lose_d3da0aff:

    # "You return to your position."
    ""

translate tranditional_chinese mino_battle_d488a282:

    # "The minotaur is standing before you, he looks enraged by your intrusion."
    ""

translate tranditional_chinese mino_battle_d5e446e1:

    # "You can feel his... member dangling in front of you, that must be what the scripture described as hard, and majestic."
    ""

translate tranditional_chinese mino_battle_4300fbe8:

    # "You raise your [pc.weapon.name!t], defending yourself from the minotaur's attack."
    ""

translate tranditional_chinese mino_battle_5a9a9a3b:

    # "You raise your fist, defending yourself from the minotaur's attack."
    ""

translate tranditional_chinese mino_battle_loop_522af612:

    # "You slash your [pc.weapon.name!t] at the arm of the minotaur, but he dodges the attack in time."
    ""

translate tranditional_chinese mino_battle_loop_cddd3861:

    # "You slam your [pc.weapon.name!t] at the minotaur's head, but he dodges the attack in time."
    ""

translate tranditional_chinese mino_battle_loop_f9c3b393:

    # "You aim and shoot your [pc.weapon.name!t] at the minotaur, but it seems to have missed."
    ""

translate tranditional_chinese mino_battle_loop_ef73b906:

    # "You throw your fist at the minotaur, but you punch into the air instead."
    ""

translate tranditional_chinese mino_battle_loop_b5605c07:

    # "You slash your [pc.weapon.name!t] at the arm of the minotaur, your blade scraps against his fur."
    ""

translate tranditional_chinese mino_battle_loop_a03754cb:

    # "The minotaur stomps on the ground, clearly protesting against your attack."
    ""

translate tranditional_chinese mino_battle_loop_f3e942f7:

    # "You slash your [pc.weapon.name!t] across the minotaur's body, knocking him back a few steps."
    ""

translate tranditional_chinese mino_battle_loop_6c4c87d5:

    # "The minotaur grunts silently in anger."
    ""

translate tranditional_chinese mino_battle_loop_229b142c:

    # "You aim and shoot your [pc.weapon.name!t] at the minotaur, the arrow hits right into his arm."
    ""

translate tranditional_chinese mino_battle_loop_a03754cb_1:

    # "The minotaur stomps on the ground, clearly protesting against your attack."
    ""

translate tranditional_chinese mino_battle_loop_5acc2781:

    # "You run while shooting your [pc.weapon.name!t] across the minotaur's body, knocking him back a few steps."
    ""

translate tranditional_chinese mino_battle_loop_6c4c87d5_1:

    # "The minotaur grunts silently in anger."
    ""

translate tranditional_chinese mino_battle_loop_057bbe22:

    # "You throw your fist at the arm of the minotaur, your blade scraps against his fur."
    ""

translate tranditional_chinese mino_battle_loop_a03754cb_2:

    # "The minotaur stomps on the ground, clearly protesting against your attack."
    ""

translate tranditional_chinese mino_battle_loop_be17f835:

    # "You punch into the minotaur's stomach, knocking him back a few steps."
    ""

translate tranditional_chinese mino_battle_loop_6c4c87d5_2:

    # "The minotaur grunts silently in anger."
    ""

translate tranditional_chinese mino_battle_loop_24bcc736:

    # "His health decreases by [oa[4]] HP."
    ""

translate tranditional_chinese mino_battle_loop_aafc665e:

    # "You've critically hit the minotaur, dealing [oa[4]] HP!"
    ""

translate tranditional_chinese mino_battle_loop_241c58ed:

    # "You struggle against the arm of the minotaur, trying to break free. You dealt [oa[4]] damage to the minotaur in the process, his grip has loosen as well."
    ""

translate tranditional_chinese mino_battle_loop_c9974a20:

    # "You struggle against the minotaur as you try to reach under the minotaur's crotch, trying to get a reaction from the minotaur."
    ""

translate tranditional_chinese mino_battle_loop_471bd576:

    # "The minotaur instanly react with your advance, grunting profusely as his cock twitches. His grip is weakening as well."
    ""

translate tranditional_chinese mino_battle_loop_fdf9de27:

    # "You turn around and rub your hand all over your own burly cheeks, feeling and brushing against your ass while you shake your hip."
    ""

translate tranditional_chinese mino_battle_loop_b3fc0b8d:

    # "You scrape your member lightly, running your claw from your inner thigh to the back of your balls, you tug at it tightly while staring at the Rune minotaur seductively."
    ""

translate tranditional_chinese mino_battle_loop_20199561:

    # "You cup at your fluffy chest, drawing circles around the area of your nipples. You smile at the Rune minotaur while your chest bounce up and down slightly."
    ""

translate tranditional_chinese mino_battle_loop_a37a7ed0:

    # "You approach to the minotaur closely, letting your hands wander inside the huge bush in his crotch area."
    ""

translate tranditional_chinese mino_battle_loop_959a28a6:

    # "You continue your act for about a minute, but the minotaur doesn't even flinch."
    ""

translate tranditional_chinese mino_battle_loop_f96ea304:

    # "Disappointed, you back away before the minotaur can grab a hold of you."
    ""

translate tranditional_chinese mino_battle_loop_7af37212:

    # "Within a few seconds you can already see some movements in his cock."
    ""

translate tranditional_chinese mino_battle_loop_eae9c4ee:

    # "The minotaur doesn't say anything, except for licking his lips. His lust is increased by [player_flirt]."
    ""

translate tranditional_chinese mino_battle_loop_05de855b:

    # mn "...H...Hnnnnngh..."
    mn ""

translate tranditional_chinese mino_battle_loop_bcf9859c:

    # "You notice the minotaur is floundering, trying his best not to get aroused by your seduction."
    ""

translate tranditional_chinese mino_battle_loop_31d7e065:

    # "But it is evident that his flushed face tells it all. His lust is increased by [player_flirt]."
    ""

translate tranditional_chinese mino_battle_loop_e593565c:

    # mn "No... more... cum..."
    mn ""

translate tranditional_chinese mino_battle_loop_d8052ded:

    # "The minotaur is squirming in reaction to your advance."
    ""

translate tranditional_chinese mino_battle_loop_aa7d256f:

    # "You can already hear his rapid breathing and grunting. His lust is increased by [player_flirt]."
    ""

translate tranditional_chinese mino_battle_loop_9ce31d8e:

    # mn "Hnnngh... As..ass...?"
    mn ""

translate tranditional_chinese mino_battle_loop_ada73862:

    # "You can tell the minotaur is already playing with himself when his hand nudges against his erect cock, staring at your ass intently."
    ""

translate tranditional_chinese mino_battle_loop_877a56ed:

    # "His lust is increased by [player_flirt]."
    ""

translate tranditional_chinese mino_battle_loop_a2c49fea:

    # mn "No..."
    mn ""

translate tranditional_chinese mino_battle_loop_4570dce2:

    # "You can tell by the minotaur's fury, he now deals much more damage, as his lust increases."
    ""

translate tranditional_chinese mino_battle_loop_77197b5f:

    # "As much as you try, you cannot escape from the barrier behind you."
    ""

translate tranditional_chinese mino_battle_loop_a9c8dad7:

    # "You fall to your knees, exhausted all your energy, you grasp for breath as you lie on the ground, surrendering yourself to the minotaur."
    ""

translate tranditional_chinese mino_battle_loop_386704da:

    # mn "U-ughh...not a-again..."
    mn ""

translate tranditional_chinese mino_battle_loop_644be555:

    # "The minotaur falls with a loud thud on the ground."
    ""

translate tranditional_chinese mino_battle_loop_92b59618:

    # "The minotaur holds you in place with his right arm. You try to struggle free but it doesn't work."
    ""

translate tranditional_chinese mino_battle_loop_0a75f7de:

    # "The minotaur swing his fist toward your direction, hitting you in the chest. Your health decreases by [enemy_damage] HP."
    ""

translate tranditional_chinese mino_battle_loop_48f7c770:

    # "The minotaur strike you down with his palm, you pass out for a few seconds before getting up. Your health decreases by [enemy_damage] HP."
    ""

translate tranditional_chinese mino_battle_loop_febb9f14:

    # "The minotaur swing his fist toward your direction, but you manage to dodge his blow."
    ""

translate tranditional_chinese mino_battle_loop_ecd5dd80:

    # "The minotaur tries to strike you down with his palm, but he missed the attack just by inches."
    ""

translate tranditional_chinese mino_battle_loop_b7a71d54:

    # "The minotaur chants and sings a warsong, his damage seems to have been increased."
    ""

translate tranditional_chinese mino_battle_loop_8d6713cf:

    # "You can still feel yourself being wrapped around by the minotaur's strong arm, refusing to let you go."
    ""

translate tranditional_chinese mino_battle_loop_a8885461:

    # "He buries your face into his warm and fuzzy chest, your lust has increased by 9."
    ""

translate tranditional_chinese mino_win_2d2ab173:

    # "You have won against the minotaur."
    ""

translate tranditional_chinese mino_win_595d80d7:

    # "You take out the vial that Haskell asked you to bring..."
    ""

translate tranditional_chinese mino_win_81a696e3:

    # "You retrieved a bottle of minotaur's essence."
    ""

translate tranditional_chinese mino_win_9a89312e:

    # "You also found [gold_drop] gold and [exp_drop] EXP."
    ""

translate tranditional_chinese mino_win_91f1d522:

    # msg "You are now Level [pc.level]! Check your inventory to allocate your level points."
    msg ""

translate tranditional_chinese mino_win_5d236da7:

    # "You leave the dungeon soon after."
    ""

translate tranditional_chinese mino_lose_7240b865:

    # "You lost against the minotaur...he seems to be very satisfied with your slumped form."
    ""

translate tranditional_chinese strings:

    old "goat"
    new ""

    old "bear"
    new ""

    old "Do you want to play the lose scene?"
    new ""

    old "Yes{#mimiclose}"
    new ""

    old "Skip{#mimiclose}"
    new ""

    old "Rune Guardian"
    new ""

    old "Should you use the rock you picked up on the guardian?"
    new ""

    old "Yes{#userockonguardian}"
    new ""

    old "No{#userockonguardian}"
    new ""

    old "Inform Kari of the Chief's safety."
    new ""

    old "Back to Main Menu"
    new ""

    old "Restart Dungeon"
    new ""

    old "Do you want to play the scene?"
    new ""

    old "Yes{#fuckmino}"
    new ""

    old "No{#fuckmino}"
    new ""

    old "Yes{#minofuck}"
    new ""

    old "No{#minofuck}"
    new ""

